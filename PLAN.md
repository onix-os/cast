...
**Current-plan boundary:** on 2026-07-23 the user ended further Phase 11
expansion and directed repository closure. The completed boundary includes the
reviewed recovery foundations and commit `ce060c13`, which moves live
ActiveReblit verification onto the durable coordinator, completes its no-boot
tail through exact terminal deletion and clean admission, production-wires the
receipt-backed boot tail, and proves the no-boot route through real
`Client::verify(true, false)`. Fresh-state and archived-state live migration,
boot-applicable end-to-end proof, production boot repair, and the remaining
reboot/power-loss VM campaign are explicitly recorded in
[`FUTURE_PLAN.md`](FUTURE_PLAN.md#deferred-state-activation-closure). They are
not claimed complete and no longer block closure of this plan.
...
journal absence; ActivateArchived reaches `RollbackComplete` and finalizes through separate bounded entries; ActiveReblit rollback with no required boot repair advances from `CandidatePreserved` to `RollbackComplete`, then finalizes separately.
A rollback from `BootSyncStarted` still routes `CandidatePreserved` to `BootRepairRequired`; no production path performs boot repair. Fresh startup at `BootRepairStarted` invokes boot zero times and retains terminal `BootRepairUnverified` evidence.
Commits `406cabe5` and `ffc32ce1` retain the explicit `BootRepairComplete` vocabulary and route an already-durable successful repair record to `RollbackComplete`, but no production path emits that successful repair record.
Accepted commit `591e8fd7` adds a separate exact promoted-receipt-backed ActiveReblit `BootSyncComplete` startup authority over the bound source record, complete state/provenance, selected active state, reservation, and namespace evidence.
Accepted commit `facd9729` dispatches that checkpoint before replacement cleanup, root-ABI normalization, or rollback admission. Ready authority derives and persists exactly one bound `CommitDecided` successor, authenticates it in the same store, after canonical reopen against the old binding, and again through a fresh binding.
Every handled entry immediately returns `RecoveryPending`; the new successor is never cleaned up or redispatched in the same entry. Unpromoted, stable selection-mismatched, and valid legacy v2 receipt-less `BootSyncComplete` evidence remains exactly pending and never falls into rollback.
The isolated promoted-receipt prerequisite passed 20/20. The new leaf gate passed 6 authority + 4 dispatch + 5 evidence-race + 1 ten-case storage-fault matrix + 2 namespace-classifier tests; `make check` and the source-LOC gate passed.
The full focused aggregate dependency remains uncredited because unrelated existing two-second Gluon deadline tests flaked under cumulative load; its isolated prerequisite and the new leaf groups above are green.
Accepted commits `c0aae8bc`, `eed3d733`, and `1167383e` now carry exact promoted-receipt-backed `CommitDecided` through read-only Apply-or-Finish authority, at most one wrapper exchange, one shared fixed durability suffix, and one bound journal advance to `CommitCleanupComplete`; the successor is authenticated in the original store, after canonical reopen against the old binding, after fresh-binding capture against the old binding again, and through the fresh binding before the handled entry returns `RecoveryPending`. The 6 authority/classifier, 3 effect, and 4 persistence/dispatch tests pass, including current/historical Apply and Finish, all five journal-update faults, six same-byte/new-inode windows, and focused database/namespace races; `make check` and source-LOC pass. Commits `84f16fd7` and `68aebceb` first closed the next two journal edges with receipt-head retirement; corrective commit `686106ff` supersedes that retirement model before merge. `CommitCleanupComplete` and `Complete` now authenticate but never clear the exact promoted receipt: its committed head is the durable installed-publication pointer and the predecessor for the next update. The correction removes the retirement API and its tests, adds exact installed-head and sequential A-to-B receipt proofs, preserves the one bound `Complete` advance and one record-bound terminal deletion, and keeps immutable receipt bodies unchanged. The installed-receipt 2/2, cleanup-completion 5/5, and finalization 5/5 suites pass with `make check` and source-LOC. Forward cleanup/finalization process-death, reboot, and power-loss proof remain open, as do boot replacement/deletion and selected-payload bootability; long-term immutable-body retention and GC stays in `FUTURE_PLAN.md`.
ActiveReblit retains its legacy Intent/Exchanged-only 12-case terminal and 32-case wrapper-exchange matrices; ActivateArchived retains its legacy-only 12-case terminal and 28-case preservation matrices. Boot-completion process death, reboot, and power-loss proof remain open; every existing process-death matrix is same-boot `SIGKILL` evidence only.
...
**Deferred Phase 11 exit gate:** after a kill or power-loss-equivalent interruption at every persisted boundary, reopening Cast either completes the committed transition,
...
rollback. This broader gate now belongs to the deferred state-activation work
in `FUTURE_PLAN.md`; it is retained here as the original acceptance contract,
not as a completed claim.
