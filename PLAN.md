...
Accepted commits `c0aae8bc`, `eed3d733`, and `1167383e` now carry exact promoted-receipt-backed `CommitDecided` through read-only Apply-or-Finish authority, at most one wrapper exchange, one shared fixed durability suffix, and one bound journal advance to `CommitCleanupComplete`; the successor is authenticated in the original store, after canonical reopen against the old binding, after fresh-binding capture against the old binding again, and through the fresh binding before the handled entry returns `RecoveryPending`. The 6 authority/classifier, 3 effect, and 4 persistence/dispatch tests pass, including current/historical Apply and Finish, all five journal-update faults, six same-byte/new-inode windows, and focused database/namespace races; `make check` and source-LOC pass. Accepted commit `84f16fd7` consumes only exact v3 `CommitCleanupComplete`: Promoted admission retires the singleton current receipt head once while retaining and authenticating the current and optional predecessor immutable bodies; already-Retired admission performs no database mutation. It then publishes the sole bound `Complete` successor, proves it in the original store and through canonical reopen with old and fresh bindings, and returns `RecoveryPending` without finalization or same-entry redispatch. Its 5 focused tests cover current/historical Apply and Finish plus byte-stable `Complete` re-entry, after-commit retirement uncertainty converging through zero-mutation Finish, all five journal faults, six same-byte/new-inode windows, and focused database/namespace races; the focused Make gate, `make check`, and source-LOC pass. Accepted commit `68aebceb` then admits only exact forward ActiveReblit `Complete`, preserves exact Retired receipt state and the current and optional predecessor immutable bodies, consumes its record binding through one record-bound terminal deletion, and retains the same locked store through final public absence and clean-startup admission. Its focused 5/5 finalization tests, the prior 5/5 completion tests, `make check`, and source-LOC pass. Forward cleanup/finalization process-death, reboot, and power-loss proof remain open, as do boot replacement/deletion and selected-payload bootability; long-term immutable-body retention and GC stays in `FUTURE_PLAN.md`.
