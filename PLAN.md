...
Phase 11 remains open. Completed foundations include canonical transition IDs, no-replace merged-/usr link publication, the bounded checksummed journal, retained tree identity and marker primitives, strict startup evidence gates, database ownership probes, and an operation-typed durable RootLinks rollback prefix through `FreshDbInvalidationIntent` for NewState, `RollbackComplete` for ActivateArchived, and `CandidatePreserved` for ActiveReblit, plus descriptor-rooted
...
normalize exchange durability, persist and route rollback, reverse `/usr`, and persist `UsrRestored`; RootLinks-sourced recovery now also preserves every operation's candidate. NewState advances exact generation 15 to `FreshDbInvalidationIntent` generation 16 and stops byte-stably; ActivateArchived advances generation 11 to `RollbackComplete` generation 12 and stops because RootLinks finalization remains closed; ActiveReblit stops at `CandidatePreserved` generation 13.
...
downstream NewState RootLinks gates remain closed. Commit `9adc2760` keeps the inventory
