...
**Current-plan boundary:** on 2026-07-23 the user directed repository closure.
Commit `ce060c13` proves live ActiveReblit's durable no-boot route. Unfinished
work is recorded without a completion claim in [`FUTURE_PLAN.md`](FUTURE_PLAN.md#deferred-state-activation-closure).
...
**Deferred Phase 11 exit gate:** interruption at every persisted boundary must
converge safely without weakening atomicity, merged-/usr, trigger isolation, or
rollback. `FUTURE_PLAN.md` retains this uncompleted acceptance contract.
