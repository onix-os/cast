...
Every entry is bounded. An exact 2 x 2 x 3 ActiveReblit matrix now sends genuine same-boot `SIGKILL` across current/historical record epochs, both rollback sources, and final-PRE source retention, kernel-observed post-unlink absence, and post-directory-sync absence. A historical epoch is not a reboot simulation; no reboot or power-loss durability is claimed. ActivateArchived candidate preservation, one-entry completion, and terminal finalization are production-wired; its genuine same-boot terminal `SIGKILL` matrix is the next operation gap.
...
no reboot or power-loss durability is claimed. Commits `8c22ec67` and `cbe3679a` add reviewed ActivateArchived child-move and completion foundations; `c8c5ea41` production-wires bounded completion, and `32bf8589` adds deterministic terminal deletion plus same-lock clean handoff. Its matching real-process `SIGKILL` matrix remains next.
