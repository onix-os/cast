...
Phase 11 remains open. Completed foundations include canonical transition IDs, no-replace merged-/usr link publication, the bounded checksummed journal, retained tree identity and marker primitives, strict startup evidence gates, database ownership probes, and an operation-typed durable RootLinks rollback prefix through `CandidatePreserved` for every operation and `FreshDbInvalidationIntent` for NewState, plus descriptor-rooted
...
normalize exchange durability, persist and route rollback, reverse `/usr`, and persist `UsrRestored`; RootLinks-sourced recovery now also preserves every operation's candidate. NewState alone advances exact generation 15 to `FreshDbInvalidationIntent` generation 16 and then stops byte-stably, while ActivateArchived and ActiveReblit stop at `CandidatePreserved`.
...
journal absence, while ActiveReblit with no required boot repair advances from `CandidatePreserved` to `RollbackComplete`, then
...
At that historical checkpoint, commit `0f041afe` placed this routing authority behind its own test-only seal. Accepted commit `e35a2183` now admits the exact
RootLinks-sourced NewState record and carries its non-Clone record-inode binding through a bound generation-15 `CandidatePreserved` -> generation-16
`FreshDbInvalidationIntent` advance, same-store successor validation, old-store
destruction, and independent canonical reopen. Its focused matrices cover 24
successes, 120 storage faults, 96 binding substitutions, 48 fresh-handle reopens, and 240 all-five-root-ABI races at two revalidation seams. These
reopens do not simulate process death. The end-to-end route retains one
reverse exchange; this journal-only entry performs no database mutation or other non-journal effect, trigger, boot,
completion, or finalization, and leaves generation 16 byte-stable because the
downstream RootLinks gates remain closed. Commit `9adc2760` keeps the inventory
gates equivalent while avoiding the host argument-size limit.
