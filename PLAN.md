...
Commit `03c5fd13` adds the independently reviewed production in-process `UsrExchanged` -> `RootLinksComplete` transition: it consumes the exact bound predecessor, publishes the retained no-replace root ABI once, revalidates every authority, conditionally advances only to the operation's exact successor, and retains that successor binding. Commit `a4f16351` wires exact durable `RootLinksComplete` + `POST` into startup for all three operations: a non-Clone predecessor-record binding is consumed to persist precisely one `RollbackDecided` with source `RootLinksComplete` and pending `/usr`; the already-complete root ABI is neither republished nor synchronized. Exact successor binding plus an independent canonical reopen prevent same-byte predecessor or successor replacement from becoming success, while `PRE` or incomplete-link evidence remains non-mutating. Commit `2201a24b` then admits only that exact decision through the journal-only resume route: another non-Clone record binding is consumed to persist exact `ReverseExchangeIntent`, predecessor or successor inode replacement cannot become success, all five journal fault points reopen to the exact source or successor across all three operations and both epochs, and no reverse exchange, root-ABI, database, or namespace effect occurs. A fresh entry deliberately leaves `ReverseExchangeIntent` byte-identical because the reverse-effect authority does not yet admit `RootLinksComplete`; carrying exact record identity through that complete effect, durability, and persistence chain is next.
