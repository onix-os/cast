...
Phase 11 remains open. Completed foundations include canonical transition IDs, no-replace merged-/usr link publication, the bounded checksummed journal and its exact record-bound terminal-delete store primitive, retained tree identity and marker primitives, strict startup evidence gates, database ownership probes, and an operation-typed durable RootLinks rollback prefix through `RollbackComplete` for NewState, ActivateArchived, and ActiveReblit, exact RootLinks finalization for NewState and ActivateArchived, plus descriptor-rooted
