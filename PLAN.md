...
post-commit and journal paths remove zero, reconcile exact `AlreadySatisfied` or source-versus-successor journal evidence, inventory raw temporaries before any recovery store opens, preserve all five root-link identities, and perform no unrelated effects. Generation 17 remains byte-stable; next is generation-17 `FreshDbInvalidated` -> generation-18 `RollbackComplete`, not terminal deletion. `SIGKILL` is not reboot or power-loss proof.
