...
same-boot evidence, not reboot or power-loss proof. Commit `7b3770b1` now captures one exact non-Clone `TransitionJournalRecordBinding` before namespace or database evidence and moves it through common NewState create/normalize/move, ActivateArchived, and ActiveReblit effect, durability, persistence-facing authority, and dispatch; this eliminates six coarse semantic loads, while preparation-only `RestartRequired` retains an opaque one-use unchanged-source authority. Its identical-bytes/different-inode proof covers 44 pre-effect + 44 post-effect + 16 restart = 104 cases across current and historical epochs and both `/usr` outcomes, with `BootSyncStarted` only for ActiveReblit. RootLinks remains excluded from candidate admission and routing; candidate writer persistence, conditional advance, and canonical reopen are intentionally unchanged and exact identity through that boundary is the next blocker. This does not claim candidate completion, boot repair, cleanup, reboot, or power-loss durability; roll-forward, durable boot publication and production boot-repair wiring, cleanup, other seams, reboot, and power-loss durability remain open.
