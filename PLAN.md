...
The completion-fault integration proof models one startup entry for an intent source and two for an initially incomplete exchanged source; complete-at-entry exchanged evidence reaches the decision in one, with 82/82 coordinator and 19/19 normalizer tests passing.
No path advances `RootLinksComplete` yet: next is an exact bound-record journal advance and in-process `UsrExchanged` -> `RootLinksComplete` coordinator, and startup must wait until that phase has a dispatcher.
