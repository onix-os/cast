...
`CommitDecided` cleanup, current receipt-head cleanup, the remaining roll-forward edges through `Complete` and finalization, boot replacement/deletion, and selected-payload bootability remain open; long-term immutable-body retention and GC stays in `FUTURE_PLAN.md`.
