...
Separate foundations provide explicit Gluon ESP/XBOOTLDR intent and retained same-deadline mountinfo/sysfs topology evidence. Commit `13a8d928` admits exactly `vfat`, requires per-mount `rw,nosuid,nodev,noexec,nosymfollow` plus superblock `rw`, retains the closed policy facts, and rejects drift across repeated observations. Commit `b8acd3d4` authenticates one borrowed destination directory through a bounded `fstat`/`fstatfs` sandwich as the Linux MSDOS filesystem family without claiming that magic alone is exact `vfat`; commit `029f0590` keeps that descriptor private inside the retained attachment, and commit `a93efe70` composes both filesystem claims in every topology pass. Commits `9c688dc6` through `28c4735b` add bounded kernel `DEVNAME` and fixed-512-sector geometry evidence to retained sysfs snapshots; commit `24d82abf` seals those parent-disk expectations to one freshly revalidated sysfs view. Commit `78b87df9` separately admits only an exact `/dev` root attachment reported as `devtmpfs`, with consistent read mode and device semantics, without treating mountinfo as descriptor authority. Commit `dceab6cc` now authenticates a borrowed directory as stable same-mount `TMPFS_MAGIC` descriptor evidence agreeing with that exact mountinfo policy, while explicitly not claiming the descriptor is the `/dev` root or proving whole-root bind provenance; binding it to the retained exact `/dev` attachment remains open. Commit `5ed70923` adds the strict pure GPT profile, and commit `c2539d7f` strengthens it to two complete exactly matching table passes under one cumulative ledger and deadline, returning selected ESP/XBOOTLDR geometry plus a role-independent table fingerprint without retaining table bytes or read authority. Commit `215b9032` also retains the selected partition number, logical-block size, and complete image length and adds a private same-deadline inter-pass checkpoint after the first stable table pass and before any second-pass source observation. Commit `2eeaa22c` adds bounded pure reconciliation of matching opening and closing block-node observations against those GPT scalars and one sealed sysfs expectation. Commit `f8a5da34` adds path-free read-only block observations, bounded positional reads, and a one-shot `getdents64` source over one caller-owned fresh directory description. Commit `1f9d578a` orders an already-retained block descriptor's opening preflight, GPT pass one, caller-owned same-deadline name rebind, exact inter-pass observation, GPT pass two, closing observation, and reconciliation into distinct live read-provenance evidence; opening the sealed `DEVNAME` beneath retained `/dev` and supplying the real rebind remain open. Commit `71ee5e95` reserves namespace descriptor capacity before ownership-transferring callbacks and guarantees bounded LIFO release, preparing the pure classifier for the still-open retained destination observer. Commit `9ac34286` classifies every rendered destination as stably absent, byte-exact, or different while rejecting FAT aliases, cross-mount traversal, namespace/content races, and over-budget work. Write authority, publication ordering, durability, and the VM-backed physical evidence campaign remain open. The `nosymfollow` requirement makes future boot-publication admission effectively Linux 5.10 or newer while generic `linux_fs` facilities retain the Linux 5.6 baseline. Commit `aa341706` completes the bounded deterministic BLS renderer and binds its exact authenticated inputs and sealed source catalog to the topology-aware pure publication plan without destination or mutation authority; the durable publisher and real ESP/BOOT publication remain open.
