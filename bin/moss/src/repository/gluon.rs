...
/// Pure definitions imported by authored fragments as `moss.repository.v1`.
...
impl GluonCodec for RepositoryCodec {
    ...
    fn decode(
        ...
        policy.insert_embedded_module("moss.repository.v1", GLUON_REPOSITORY_ABI)?;
...
mod tests {
    ...
    fn authored(body: &str) -> String {
        format!("let moss = import! moss.repository.v1\n{body}")
    }
    ...
    moss.repository.root_index "volatile" "https://packages.example.test" "stream/volatile",
]"#,
            ),
        );
        ...
        assert!(
            loaded[0]
                ...
                .imported_modules
                .iter()
                .any(|module| module.logical_name == "moss.repository.v1")
        );
    }
    ...
    fn generated_save_is_deterministic_and_loadable() {
        let source = authored(
            r#"moss.repositories [
    moss.repository.root_index "z-root" "https://packages.example.test" "stream/volatile",
    moss.repository.direct "a-direct" "file:///var/cache/local.index",
]"#,
        );
    ...
    fn malformed_and_conversion_errors_include_fragment_and_field_paths() {
        ...
        write(
            ...
            &authored(
                r#"moss.repositories [
    moss.repository.direct "broken" "not a url",
]"#,
            ),
        );
        let error = manager.load_gluon(&Evaluator::default(), &RepositoryCodec).unwrap_err();
    ...
    fn duplicate_ids_are_rejected_at_the_second_path() {
        ...
        write(
            ...
            &authored(
                r#"moss.repositories [
    moss.repository.direct "same/id" "file:///one.index",
    moss.repository.direct "same_id" "file:///two.index",
]"#,
            ),
        );
    ...
    fn repeated_manager_loads_have_the_same_fingerprint() {
        ...
        write(
            ...
            &authored(r#"moss.repositories [moss.repository.direct "local" "file:///local.index"]"#),
        );
