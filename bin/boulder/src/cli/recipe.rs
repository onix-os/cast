...
const LONG_UPDATE_ABOUT: &str = concat!(
    ...
    "With no --ver or --upstream values, Boulder resolves the current authored\n",
...
fn require_manual_edit(path: &Path, changes: Vec<SuggestedChange>) -> Result<(), Error> {
    ...
    if lock_refresh_required {
        ...
        println!(
            "    action = \"after applying the upstream edits, run `boulder recipe update` without update values\""
        );
        println!("  }}");
    }
    println!("}}");
...
pub enum Error {
    ...
    #[error(
        "generated source lock {path:?} is stale; run `boulder recipe update` without update values to atomically refresh it"
    )]
    StaleGeneratedLock {
        path: PathBuf,
        #[source]
