...
jq -e --arg commit "$fake_commit" '
    ...
    and .fixtures[18].name == "userspace-profile"
    and ([.fixtures[].artifacts.stone_count] | add) == 124
    and ([.fixtures[].artifacts.manifest_count] | add) == 38
    and ([.fixtures[].artifacts.artifact_count] | add) == 162
    and .result == "passed"
' "$proof" >/dev/null
