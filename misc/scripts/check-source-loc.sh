...
is_generated_marker_file() {
    # Machine-generated declaration data (the shipped policy, generated system
    # snapshots) carries an explicit generated marker on its first line. Such
    # files are emitted, not hand-authored, so they fall outside the source
    # budget in the same spirit as lock files. Match on the original,
    # case-preserved path so the file can actually be read.
    local absolute=${repo_root}/${1}
    [[ -f "${absolute}" ]] || return 1
    local first_line
    IFS= read -r first_line < "${absolute}" || return 1
    case "${first_line}" in
        *"@generated by os-tools"* | *"@generated by cast"*)
            return 0
            ;;
    esac
    return 1
...
is_owned_text() {
    ...
    is_generated_marker_file "${1}" && return 1
    case "${path}" in
