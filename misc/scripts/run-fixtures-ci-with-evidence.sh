...
stop_owned_unit() {
    ...
    environment=$(timeout --kill-after=1s 10s systemctl --user show "$unit" \
        --property=Environment --value 2>/dev/null) || return 0
    [[ -n $environment ]] || return 0
    case " $environment " in
        *" $unit_marker "*) ;;
        *)
            printf 'refusing to stop fixture CI unit without this invocation marker: %s\n' \
                "$unit" >&2
            return 0
            ;;
    esac
    ...
    if ! timeout --kill-after=1s -- "${kill_after_seconds}s" \
        ...
        printf 'normal stop failed for fixture CI unit %s; forcing its control group down\n' \
            "$unit" >&2
...
trap - EXIT
setsid timeout --kill-after="${kill_after_seconds}s" -- "${client_timeout_seconds}s" \
    systemd-run \
    --user \
    --unit="$unit" \
    --wait \
