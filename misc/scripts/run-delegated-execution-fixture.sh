...
case "$1" in
    ...
    *)
        printf '%s\n' \
            'argument must be exactly `--preflight-only`, `all`, or one of: autotools autotools-options cargo cargo-features cargo-vendored cmake custom daemon-generated desktop-integration factory-override font-family generated-config generated-shell gettext-localization header-only-library hooks-patch meson multiple-sources plugin-output post-install-smoke-test split system-integration-assets userspace-profile' >&2
        exit 2
        ;;
esac
