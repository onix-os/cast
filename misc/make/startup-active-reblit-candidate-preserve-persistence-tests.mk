...
forge-startup-usr-rollback-active-reblit-candidate-preserve-persistence-test:
	...
	for name in \
		...
		production_exclusion::startup_active_reblit_candidate_preserve_persistence_production_dispatch_remains_unsupported_without_advance; do \
	...
	negative=crates/forge/src/client/startup_recovery/usr_rollback_active_reblit_candidate_preserve_persistence/tests/production_exclusion.rs; \
	timeout 10s awk 'previous == "#[cfg(test)]" && $$0 == "mod usr_rollback_active_reblit_candidate_preserve_persistence;" { found = 1 } { previous = $$0 } END { exit !found }' "$$recovery_root"; \
	...
	timeout 10s test "$$( timeout 10s grep -c . <<<"$$production_refs" )" = 1; \
	timeout 10s test "$$( timeout 10s grep -Fc "$$recovery_root:" <<<"$$production_refs" )" = 1; \
	if timeout 10s grep -Fq 'persist_usr_rollback_active_reblit_candidate_preserve_and_reopen' "$$production_dispatch"; then exit 1; fi; \
	...
	timeout 10s grep -Fq 'UsrRollbackCandidatePreserveDispatchError::Unsupported' "$$negative"; \
	timeout 10s test "$$( timeout 10s rg -n '^#\[test\]$$' "$$matrix" "$$races" "$$storage" "$$restart" "$$negative" | timeout 10s wc -l )" = 10; \
	...
	for file in "$$executor" "$$recovery_root" "$$production_dispatch" "$$authority_root" "$$authority_active" "$$authority_post" "$$authority_persistence" "$$proof" "$$capture" "$$namespace" "$$namespace_post" "$$reconciliation_root" "$$tests" "$$support" "$$matrix" "$$races" "$$storage" "$$restart" "$$negative" misc/make/startup-active-reblit-candidate-preserve-persistence-tests.mk Makefile; do \
