...
forge-linux-fs-test: forge-linux-descriptor-boot-filesystem-test forge-linux-descriptor-boot-namespace-test forge-linux-gpt-partition-role-test forge-linux-mount-attachment-test forge-linux-mount-boot-policy-test forge-linux-mount-devtmpfs-policy-test forge-linux-mountinfo-parser-test forge-linux-mount-namespace-test forge-linux-authenticated-mountinfo-snapshot-test forge-linux-sysfs-block-parser-test forge-linux-sysfs-identity-test
	@set -eu; \
	listed="$$( timeout 300s $(CARGO) test -p forge --lib -- --list )"; \
	timeout 10s grep -q . <<<"$$listed"; \
	count="$$( timeout 10s grep -c '^linux_fs::tests::.*: test$$' <<<"$$listed" )"; \
	timeout 10s test "$$count" = 276; \
	for test in \
