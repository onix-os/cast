...
forge-linux-fs-test: forge-linux-mount-attachment-test forge-linux-mountinfo-parser-test forge-linux-mount-namespace-test forge-linux-authenticated-mountinfo-snapshot-test forge-linux-sysfs-block-parser-test forge-linux-sysfs-identity-test
	@set -eu; \
	listed="$$( timeout 300s $(CARGO) test -p forge --lib -- --list )"; \
	timeout 10s grep -q . <<<"$$listed"; \
	count="$$( timeout 10s grep -c '^linux_fs::tests::.*: test$$' <<<"$$listed" )"; \
	timeout 10s test "$$count" = 177; \
	for test in \
