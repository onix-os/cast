...
forge-linux-gpt-partition-role-test: host-storage-safety-test
	...
	for name in \
		...
		stable::stable_512_byte_esp_image_returns_only_selected_semantics \
		stable::stable_4096_byte_xbootldr_image_returns_only_selected_semantics \
	...
	for part in constants guid reader parser; do timeout 10s test -f "$$module_dir/$$part.rs"; done; \
	for part in support stable malformed bounds; do timeout 10s test -f "$$tests_dir/$$part.rs"; done; \
	...
	for part in constants guid reader parser; do timeout 10s grep -Fqx "mod $$part;" "$$module"; done; \
	for part in support stable malformed bounds; do timeout 10s grep -Fqx "mod $$part;" "$$tests"; done; \
	...
	timeout 10s grep -Fq 'pub(super) trait Image {' "$$module_dir/reader.rs"; \
