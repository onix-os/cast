...
forge-active-reblit-installed-boot-publication-delta-test: host-storage-safety-test
	...
	for name in \
		...
		tests::different_desired_without_authenticated_owned_predecessor_fails_closed \
		tests::owned_marker_without_installed_identity_fails_closed \
		tests::alias_and_distinct_keys_use_fat_folding_without_cross_root_confusion \
		tests::only_strict_empty_or_promoted_database_state_can_form_installed_input \
		tests::authenticated_claim_mapping_keeps_first_adoption_borrowed \
		tests::receipt_claim_bridge_uses_inventory_order_and_ignores_stale_entries \
		tests::receipt_claim_bridge_rejects_missing_duplicate_and_stale_desired_keys \
		tests::receipt_claim_bridge_rejects_a_desired_action_with_no_desired_key \
		effect_schedule::tests::closed_desired_dispatch_requires_exact_old_and_new_identities \
