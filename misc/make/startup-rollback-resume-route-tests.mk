...
forge-startup-usr-rollback-resume-route-test:
	...
	timeout 10s test "$$( timeout 10s grep -Fc 'Phase::RollbackComplete' "$$root_links_endpoint" )" = 3; \
	timeout 10s test "$$( timeout 10s grep -Fc 'assert_eq!(retained_exchange_syscall_count(), 1, "{case}");' "$$root_links_endpoint" )" = 9; \
	timeout 10s test "$$( timeout 10s grep -Fc 'assert_eq!(root_link_snapshot(&fixture), root_links_before, "{case}");' "$$root_links_endpoint" )" = 9; \
