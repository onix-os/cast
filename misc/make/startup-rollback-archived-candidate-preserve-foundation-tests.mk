...
forge-startup-usr-rollback-archived-candidate-preserve-foundation-test:
	...
	timeout 10s grep -Fq 'super::require_journal_record_binding(' "$$authority"; \
	if timeout 10s rg -n 'TransitionJournalBinding|journal\.binding\(\)|journal\.has_binding\(|journal\.load\(\)|journal\.advance\(' "$$authority_root" "$$authority" "$$proof" "$$proof_post"; then exit 1; else status="$$?"; timeout 10s test "$$status" = 1; fi; \
