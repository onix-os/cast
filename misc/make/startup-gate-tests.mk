forge-client-startup-gate-test:
	...
	timeout 10s awk '$$0 == "                let mut mutation_authority =" { state = 1; next } state == 1 && $$0 == "                    startup_reconciliation::ActiveReblitReplacementMutationAuthorityProvider::new(" { state = 2; next } state == 2 && $$0 == "                        &mutation_seal," { state = 3; next } state == 3 && $$0 == "                        installation," { state = 4; next } state == 4 && $$0 == "                        &journal," { state = 5; next } state == 5 && $$0 == "                        state_db," { state = 6; next } state == 6 && $$0 == "                        active_state_reservation," { state = 7; next } state == 7 && $$0 == "                        &record," { state = 8; next } state == 8 && $$0 == "                        in_flight.clone()," { state = 9; next } state == 9 && $$0 == "                    );" { found = 1 } END { exit !found }' "$$startup_gate"; \
	...
	timeout 10s awk '$$0 == "                transition_identity::recover_active_reblit_replacement_residue(&mut mutation_authority)?;" { state = 1; next } state == 1 && $$0 == "            }" { state = 2; next } state == 2 && $$0 == "" { state = 3; next } state == 3 && $$0 == "            let decision_seal = UsrRollbackDecisionSeal::new();" { found = 1 } END { exit !found }' "$$startup_gate"; \
