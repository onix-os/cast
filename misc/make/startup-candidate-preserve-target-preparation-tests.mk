...
forge-startup-usr-rollback-candidate-preserve-target-preparation-test:
	...
	timeout 10s awk '$$0 == "    fn into_apply_effect_selection(" { active = 1; next } active && $$0 == "        self.require_journal_record_binding(journal)?;" { found = 1; exit } active && ($$0 ~ /self\.namespace/ || $$0 ~ /self\.installation/ || $$0 ~ /inspect_current_database/) { exit 1 } END { exit !found }' "$$authority"; \
