...
die() {
    printf 'disposable VM GPT aggregate runner rejected invocation: %s\n' "$*" >&2
    exit 2
}
...
[ "${CAST_VM_GPT_TOPOLOGY_CONFIRMATION-}" \
    = disposable-vm-gpt-receipt-bound-aggregate-only ] \
    || die 'dedicated GPT aggregate confirmation is absent'
...
expected_commit=$(required CAST_VM_BOOT_PUBLICATION_EXPECTED_COMMIT)
case "$expected_commit" in
    *[!0-9a-f]* | '') die 'expected source commit is invalid' ;;
esac
case "${#expected_commit}" in
    40 | 64) ;;
    *) die 'expected source commit length is invalid' ;;
esac
...
[ "$(/usr/bin/grep -Fxc 'protocol=3' "$marker")" = 1 ] \
    || die 'consumed marker protocol is not aggregate-specific'
[ "$(/usr/bin/grep -Fxc 'campaign_profile=gpt-receipt-bound-aggregate-v1' "$marker")" = 1 ] \
    || die 'consumed marker profile is not aggregate-specific'
...
[ "$(/usr/bin/grep -Fxc "commit=$expected_commit" "$marker")" = 1 ] \
    || die 'consumed marker source-commit binding changed'
...
fixture_parent=$(required CAST_VM_GPT_AGGREGATE_FIXTURE_PARENT)
[ "$fixture_parent" = "$build_root/gpt-aggregate-fixtures/$kind" ] \
    || die 'aggregate fixture parent is not marker- and topology-bound'
[ -d "$build_root/gpt-aggregate-fixtures" ] \
    && [ ! -L "$build_root/gpt-aggregate-fixtures" ] \
    && [ "$(/usr/bin/stat -Lc '%u:%g:%a:%F' \
        -- "$build_root/gpt-aggregate-fixtures")" = '0:0:700:directory' ] \
    || die 'aggregate fixture root metadata is unsafe'
[ -d "$fixture_parent" ] && [ ! -L "$fixture_parent" ] \
    && [ "$(/usr/bin/stat -Lc '%u:%g:%a:%F' -- "$fixture_parent")" \
        = '0:0:700:directory' ] \
    || die 'aggregate fixture parent metadata is unsafe'
...
test_name='client::disposable_vm_gpt_aggregate_publication_tests::disposable_vm_receipt_bound_aggregate_publication'
cd /
test_status=0
