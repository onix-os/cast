...
- [ ] Open mutable system clients in recovery order: installation lock,
  ...
  Caller-supplied provenance claims are not authenticated effect authority. Accepted commits `4d1f8ceb` and `12f888e1` now supply the receipt-bound aggregate attempt and disposable-UEFI-VM GPT alias/distinct remount evidence; accepted commit `323f776e` separately supplies exact database-only pending-to-committed promotion. Terminal aggregate evidence is not yet allowed to invoke that primitive, and journal persistence to `BootSyncComplete`, live coordinator/startup wiring, replacement/deletion, selected-payload bootability, reboot, and power-loss proof remain open.
...
Accepted commits `4d1f8ceb` and `12f888e1` close the immutable aggregate-attempt and disposable-UEFI-VM GPT alias/distinct remount-evidence gaps; accepted commit `323f776e` closes only the database promotion substrate with 14/14 focused tests. Authenticated terminal-evidence consumption, `BootSyncComplete`, same-receipt recovery, live wiring, boot replacement/deletion authority, selected-payload bootability, reboot, and power-loss proof remain open.
...
Accepted commits `4d1f8ceb`, `12f888e1`, and `323f776e` now provide the immutable aggregate attempt, real GPT alias/distinct remount evidence, and a separate database-only promotion substrate. Authenticated aggregate-result consumption, `BootSyncComplete`, replacement/deletion, live wiring, selected-payload bootability, same-receipt recovery, reboot, and power-loss durability remain open.
