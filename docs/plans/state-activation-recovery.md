...
- [ ] Open mutable system clients in recovery order: installation lock,
  ...
  Exact startup roll-forward is production-wired through `BootSyncComplete` -> `CommitDecided` -> `CommitCleanupComplete` -> `Complete` -> authenticated clean startup, with cleanup, receipt retirement, and record-bound terminal deletion attempted at most once in their separate entries and every handled edge returning immediately;
...
Accepted commits `4d1f8ceb`, `12f888e1`, `323f776e`, `fffb79f7`, and `58235570` close the aggregate, VM remount, promotion, terminal-consumption, and exact `BootSyncComplete` foundation. Accepted commits `591e8fd7` and `facd9729` authenticate exact promoted-receipt-backed startup completion and advance it once to bound `CommitDecided`, with same-store and canonical-reopen binding proof before immediate `RecoveryPending`. Accepted commits `c0aae8bc`, `eed3d733`, and `1167383e` carry that exact record through read-only cleanup authority, one Apply exchange or zero-exchange Finish, the shared durability suffix, and one bound `CommitCleanupComplete` advance with old/fresh binding and complete retained-evidence proof. Cleanup gates pass 6 + 3 + 4 tests; `make check` and source-LOC pass. Accepted commit `84f16fd7` then authenticates exact Promoted-or-Retired receipt state and the completed cleanup namespace, performs at most one singleton-head retirement while retaining immutable bodies, publishes one bound `Complete` successor with same-store and canonical-reopen old/fresh binding proof, and returns `RecoveryPending`. Its 5 focused tests cover current/historical Apply/Finish and stable re-entry, retirement uncertainty, five journal faults, six inode-substitution windows, and database/namespace races. Accepted commit `68aebceb` authenticates only exact forward ActiveReblit `Complete`, preserves exact Retired receipt state and immutable bodies, consumes one record-bound terminal deletion, and retains the same locked store through final absence and clean admission. Its focused 5/5 finalization tests plus the prior 5/5 completion tests, `make check`, and source-LOC pass. Forward cleanup/finalization process death, reboot, and power-loss proof remain open, as do boot replacement/deletion and selected-payload bootability; immutable-body GC stays in `FUTURE_PLAN.md`.
...
Accepted commits `4d1f8ceb`, `12f888e1`, `323f776e`, `fffb79f7`, and `58235570` provide the aggregate, VM remount evidence, promotion, terminal consumption, and exact `BootSyncComplete` foundation. Accepted commits `591e8fd7` and `facd9729` authenticate and dispatch its one-shot bound advance to `CommitDecided`; accepted commits `c0aae8bc`, `eed3d733`, and `1167383e` then authenticate exact cleanup, perform at most one Apply exchange or zero-exchange Finish, complete the fixed durability suffix, and publish one bound `CommitCleanupComplete` successor before immediate `RecoveryPending`. Both advances retain same-store, canonical-reopen old-binding, repeated old-binding, and fresh-binding proof. Cleanup gates pass 6 authority/classifier + 3 effect + 4 persistence/dispatch tests; `make check` and source-LOC pass. Accepted commit `84f16fd7` closes the exact next edge: Promoted admission retires the current singleton receipt head once, Retired admission mutates no database state, both authenticate the retained current and optional predecessor immutable bodies, and one bound advance publishes `Complete` with same-store and canonical-reopen binding proof before immediate `RecoveryPending`. Its 5 focused tests cover current/historical Apply and Finish, stable `Complete` re-entry, committed retirement uncertainty, all five journal faults, all six binding windows, and database/namespace races. Accepted commit `68aebceb` then finalizes only exact forward ActiveReblit `Complete` through one record-bound terminal deletion while preserving exact Retired receipt state and immutable bodies under the same locked-store clean admission. Its focused 5/5 finalization tests plus the prior 5/5 completion tests, `make check`, and source-LOC pass. Forward cleanup/finalization process death, reboot, and power-loss proof remain open, as do boot replacement/deletion and selected-payload bootability; immutable-body GC stays in `FUTURE_PLAN.md`.
...
Accepted commits `591e8fd7` and `facd9729` supersede the startup-adoption limitation by advancing exact promoted-receipt-backed ActiveReblit `BootSyncComplete` once to bound `CommitDecided`; accepted commits `c0aae8bc`, `eed3d733`, and `1167383e` continue that production ladder through exact one-shot cleanup and a bound `CommitCleanupComplete` successor. Accepted commit `84f16fd7` retires the exact current receipt head at most once, preserves its immutable bodies, and advances once to bound `Complete`; every one of those handled edges ends in `RecoveryPending`. Its 5 focused tests cover current/historical Apply/Finish, stable re-entry, retirement uncertainty, five journal faults, six binding windows, and database/namespace races. Accepted commit `68aebceb` finalizes only exact forward `Complete` through one record-bound terminal deletion and same locked-store clean admission while preserving exact Retired receipt/body evidence. Its focused finalization gate passes 5/5 alongside the prior 5/5 completion gate, `make check`, and source-LOC. Forward cleanup/finalization process death, reboot, and power-loss proof remain open, as do boot replacement/deletion and selected-payload bootability; immutable-body GC stays in `FUTURE_PLAN.md`.
