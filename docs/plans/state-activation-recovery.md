...
- [ ] Open mutable system clients in recovery order: installation lock,
  ...
  Exact startup roll-forward is production-wired through `BootSyncComplete` -> `CommitDecided` -> `CommitCleanupComplete` -> `Complete`, with cleanup and receipt retirement attempted at most once per entry and every handled edge returning immediately;
  forward `Complete` finalization, actual boot repair, replacement/deletion, cleanup process-death, reboot, and power-loss proof remain open.
...
Disk admissibility beyond the admitted evidence, forward finalization beyond exact startup `Complete`, device-flush ordering, and process-death/reboot reconciliation remain open. Every `58235570` failure description remains conditional: authority is consumed, fresh reconciliation may expose only exact `BootSyncStarted` or `BootSyncComplete`, and otherwise fails stop; `591e8fd7`/`facd9729` admit only the exact promoted completion authority, `c0aae8bc`/`eed3d733`/`1167383e` separately admit, execute, durably finish, and persist only the exact cleanup edge, and `84f16fd7` retires only its exact current head before the bound `Complete` edge described above.
...
Accepted commits `591e8fd7` and `facd9729` supersede the startup-adoption limitation by advancing exact promoted-receipt-backed ActiveReblit `BootSyncComplete` once to bound `CommitDecided`; accepted commits `c0aae8bc`, `eed3d733`, and `1167383e` continue that production ladder through exact one-shot cleanup and a bound `CommitCleanupComplete` successor. Accepted commit `84f16fd7` retires the exact current receipt head at most once, preserves its immutable bodies, and advances once to bound `Complete`; every handled edge ends in `RecoveryPending`. Its 5 focused tests cover current/historical Apply/Finish, stable re-entry, retirement uncertainty, five journal faults, six binding windows, and database/namespace races. Forward `Complete` finalization, boot replacement/deletion and bootability, cleanup process death, reboot, and power-loss proof remain open; immutable-body GC stays in `FUTURE_PLAN.md`.
