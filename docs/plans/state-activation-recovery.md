...
- [ ] Open mutable system clients in recovery order: installation lock,
  ...
  Exact startup roll-forward is production-wired through `BootSyncComplete` -> `CommitDecided` -> `CommitCleanupComplete`, with cleanup attempted at most once per entry and every handled edge returning immediately;
  current receipt-head cleanup, `CommitCleanupComplete` -> `Complete`, finalization, actual boot repair, replacement/deletion, cleanup process-death, reboot, and power-loss proof remain open.
...
Disk admissibility beyond the admitted evidence, roll-forward beyond exact startup `CommitCleanupComplete`, device-flush ordering, and process-death/reboot reconciliation remain open. Every `58235570` failure description remains conditional: authority is consumed, fresh reconciliation may expose only exact `BootSyncStarted` or `BootSyncComplete`, and otherwise fails stop; `591e8fd7`/`facd9729` admit only the exact promoted completion authority, while `c0aae8bc`/`eed3d733`/`1167383e` separately admit, execute, durably finish, and persist only the exact cleanup edge described above.
