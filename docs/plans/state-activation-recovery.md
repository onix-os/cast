...
- [ ] Open mutable system clients in recovery order: installation lock,
  ...
  Caller-supplied provenance claims are not authenticated effect authority. Accepted commits `4d1f8ceb`, `12f888e1`, `323f776e`, `fffb79f7`, and `58235570` supply the receipt-bound aggregate, VM remount evidence, promotion, terminal consumption, and exact `BootSyncComplete` foundation described below. Accepted commit `591e8fd7` adds exact promoted-receipt-backed ActiveReblit `BootSyncComplete` startup authority over the bound record, full state/provenance, selected active state, reservation, and namespace; accepted commit `facd9729` dispatches it before cleanup, root-ABI normalization, and rollback, then advances exactly once to bound `CommitDecided` with same-store, canonical-reopen old-binding, fresh-binding, and repeated old-binding proof. The handled entry returns `RecoveryPending`; legacy v2 receipt-less, unpromoted, and stable-mismatched evidence remains exactly pending. Its isolated prerequisite passes 20/20, the leaf groups pass 6 + 4 + 5 + 1 ten-case matrix + 2, and `make check` plus source-LOC pass; the full aggregate remains uncredited because unrelated existing two-second Gluon deadlines flaked under cumulative load. Later cleanup, current receipt-head cleanup, roll-forward through `Complete` and finalization, boot replacement/deletion and bootability, process death, reboot, and power-loss proof remain open; long-term immutable-body GC is deferred.
  ...
  Only the exact startup `BootSyncComplete` -> `CommitDecided` edge is production-wired; later roll-forward, `CommitDecided` cleanup, current receipt-head cleanup, `Complete`/finalization, and actual boot
  repair, replacement/deletion, process-death, reboot, and power-loss proof remain open.
...
Accepted commits `4d1f8ceb`, `12f888e1`, `323f776e`, `fffb79f7`, and `58235570` close the aggregate, VM remount, promotion, terminal-consumption, and exact `BootSyncComplete` foundation. Accepted commits `591e8fd7` and `facd9729` now authenticate exact promoted-receipt-backed ActiveReblit `BootSyncComplete` at startup, dispatch it before cleanup/root-ABI/rollback, and advance it exactly once to bound `CommitDecided`, with same-store, canonical-reopen old-binding, fresh-binding, and repeated old-binding proof before immediate `RecoveryPending`. Legacy v2 receipt-less, unpromoted, and stable-mismatched evidence remains pending. The isolated prerequisite passes 20/20; leaf groups pass 6 + 4 + 5 + 1 ten-case storage matrix + 2; `make check` and source-LOC pass. The full aggregate remains uncredited because unrelated existing two-second Gluon deadlines flaked under cumulative load. Later cleanup, current receipt-head cleanup, `Complete`/finalization, boot replacement/deletion and bootability, process death, reboot, and power-loss proof remain open.
...
Accepted commits `4d1f8ceb`, `12f888e1`, `323f776e`, `fffb79f7`, and `58235570` provide the aggregate, VM remount, promotion, terminal-consumption, and exact `BootSyncComplete` foundation. Accepted commits `591e8fd7` and `facd9729` add the exact promoted-receipt-backed startup authority and its early one-shot bound advance to `CommitDecided`; success is proved in the same store, through canonical reopen against the old binding, through a fresh binding, and through repeated old-binding proof, then returns `RecoveryPending`. The isolated prerequisite and leaf groups pass 20/20 and 6 + 4 + 5 + 1 ten-case matrix + 2; `make check` and source-LOC pass, while the full aggregate is uncredited due unrelated existing Gluon deadline flakes under cumulative load. Cleanup, current receipt-head cleanup, later roll-forward, boot replacement/deletion and bootability, process death, reboot, and power-loss durability remain open.
...
Disk admissibility beyond the admitted evidence, roll-forward beyond exact startup `CommitDecided`, device-flush ordering, and process-death/reboot reconciliation remain open. Every `58235570` failure description remains conditional: authority is consumed, fresh reconciliation may expose only exact `BootSyncStarted` or `BootSyncComplete`, and otherwise fails stop; `591e8fd7`/`facd9729` admit only the exact promoted completion authority described above.
...
Accepted commits `591e8fd7` and `facd9729` supersede only that startup-adoption limitation: exact promoted-receipt-backed ActiveReblit `BootSyncComplete` now dispatches early and advances once to bound `CommitDecided`, then returns `RecoveryPending`. Later cleanup, current receipt-head cleanup, roll-forward, boot replacement/deletion and bootability, process death, reboot, and power-loss proof remain open.
