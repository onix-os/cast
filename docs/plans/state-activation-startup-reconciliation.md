...
## Admitted startup recovery ladder
  ...
  RootLinks-sourced `UsrRestored` now routes through candidate preservation. NewState advances exact generation-15 `CandidatePreserved` through generation-16 `FreshDbInvalidationIntent` and generation-17 `FreshDbInvalidated` to generation-18 `RollbackComplete`; ActivateArchived advances generation 11 to generation-12 `RollbackComplete`; and ActiveReblit advances generation 13 to generation-14 `RollbackComplete`. Each then stops byte-stably because RootLinks terminal finalization remains closed. For the pre-existing later source set, NewState prepares its
  quarantine target, preserves the candidate, invalidates its exact fresh row,
  reaches `RollbackComplete`, and authenticates terminal journal absence. ActivateArchived reaches `RollbackComplete` and terminal journal absence through separate bounded entries.
...
## Remaining recovery campaign
  ...
  The production ladder covers the authenticated `/usr` rollback prefix and exact RootLinks rollback for all three operations through candidate preservation and their admitted later boundaries. NewState advances generation 15 through generation-16 `FreshDbInvalidationIntent` and generation-17 `FreshDbInvalidated` to generation-18 `RollbackComplete`; ActivateArchived advances generation 11 to generation-12 `RollbackComplete`; ActiveReblit advances generation 13 to generation-14 `RollbackComplete`. Each then stays byte-stable because RootLinks terminal finalization remains closed.
  Accepted commit `7457b259` proves NewState's generation-16 -> generation-17 boundary with exact binding through capture, Apply-or-Finish effect, bound advance, same-store validation, and independent reopen. Present performs at most one exact removal and joint absence zero; success/storage/binding/fresh-handle totals are 48/240/192/96. All-five-link races total 240 capture + 240 pre-effect + 120 Applied post-attempt + 240 initial-persistence + 240 final-revalidation. Accepted commit `68759ba3` adds exactly 20 RootLinks-only same-boot `SIGKILL` cases: two epochs x five real SQLite transaction seams plus five journal-update durability seams, using 15-second-bounded production-startup crash/recovery children after all parent handles are released. Recovery is the first database opener. The first four DB deaths roll back and then perform one exact `Applied` removal of a nonempty selected row; post-commit and journal deaths remove zero and converge through exact `AlreadySatisfied` or source/successor records. Post-crash raw temporary inventory precedes every recovery store open, all five root-link identities persist, and unrelated effects stay zero. Accepted commit `f2b305d4` separately captures and consumes exact generation-17 binding through one bound advance, same-store successor validation, old-store drop, and independent canonical reopen to generation-18 `RollbackComplete`. Its journal-only 48/240/192/96 base-success/storage/binding/fresh-handle cases and 480 capture/final root-ABI races add no process-death claim or non-journal effect. Commit `a3fb25d3` proves ActivateArchived generation 11 -> 12; accepted `a05997d8` plus `cfb5a70d` proves ActiveReblit generation 13 -> 14. The endpoint performs one reverse `/usr` exchange and leaves NewState generation 18, Archived generation 12, and ActiveReblit generation 14 stable. The 20-case `SIGKILL` proof remains only generation 16 -> 17 even when already-canonical successor recovery naturally continues to 18. RootLinks terminal finalization, other process-death boundaries, reboot, and power-loss proof remain open. Next is an exact record-bound terminal-deletion primitive before operation finalization.
  Accepted commit `8f391985` closes the shared store-primitive prerequisite but leaves all RootLinks terminal records byte-stable; operation-specific finalizer wiring and its residue/interruption proof remain open. The earlier source set retains the exact NewState suffix through authenticated terminal journal absence and the ActiveReblit no-boot-repair suffix through shared clean admission.
  ...
  bounded entry. Accepted commit `a3fb25d3` widens only that entry to exact RootLinks generation-11 `CandidatePreserved`, carries the exact record-inode binding through capture, bound advance, same-store validation, and canonical reopen, reaches generation-12 `RollbackComplete`, and deliberately leaves RootLinks finalization closed. Commit `32bf8589` adds its operation-specific terminal
