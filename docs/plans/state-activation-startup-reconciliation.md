...
### Historical test-sealed NewState target selection, creation, normalization, and move reconciliation
  ...
  post-commit and journal deaths remove zero, preserve exact `AlreadySatisfied` or source-versus-successor handling, inspect raw temporaries before any recovery store open, retain all five root-link identities, and perform no unrelated effects. Generation 17 remains stable; RootLinks finalization stays closed.
  This `SIGKILL` proof is not reboot or power-loss evidence and does not widen another source or operation; next is the separately admitted generation-17 `FreshDbInvalidated` -> generation-18 `RollbackComplete` edge.
