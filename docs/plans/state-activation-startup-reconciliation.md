...
## Admitted startup recovery ladder
  ...
  RootLinks-sourced `UsrRestored` now routes through candidate preservation. NewState advances exact generation-15 `CandidatePreserved` to generation-16 `FreshDbInvalidationIntent` and stops byte-stably there; ActivateArchived advances generation-11 `CandidatePreserved` to generation-12 `RollbackComplete` and stops byte-stably because its finalization gate remains closed; ActiveReblit remains at `CandidatePreserved` generation 13. No still-later RootLinks source axis is admitted for any operation. For the pre-existing later source set, NewState prepares its
...
## Remaining recovery campaign
  ...
  The production ladder covers the authenticated `/usr` rollback prefix and exact RootLinks rollback for all three operations through `CandidatePreserved`. NewState advances generation 15 to generation-16 `FreshDbInvalidationIntent`; ActivateArchived advances generation 11 to generation-12 `RollbackComplete`; ActiveReblit remains at generation-13 `CandidatePreserved`. Each then stays byte-stable because its still-later RootLinks gates remain closed.
  Commit `e35a2183` proves the NewState journal-only edge with bound non-Clone record-inode identity, same-store and independent-reopen validation, 24/120/96/48 success/storage/binding/fresh-handle cases, and 240 all-link races at two seams. Commit `a3fb25d3` proves the independent ActivateArchived journal-only edge with the same 24/120/96/48 matrix shape and 360 all-five-root-ABI mutations across three seams; exact database, archived-wrapper, state-slot, and root identities do not change. Fresh-handle reopen is not process death. Neither new entry performs a database or non-journal effect, cleanup, finalization, or boot action; RootLinks terminal finalization, process death, reboot, and power-loss durability remain open.
