...
## Admitted startup recovery ladder
  ...
  RootLinks-sourced `UsrRestored` now routes through candidate preservation. NewState advances exact generation-15 `CandidatePreserved` to generation-16 `FreshDbInvalidationIntent`; ActivateArchived advances generation 11 to generation-12 `RollbackComplete`; and ActiveReblit advances generation 13 to generation-14 `RollbackComplete`. Each then stops byte-stably because its later RootLinks gate remains closed. For the pre-existing later source set, NewState prepares its
  ...
  Commit `67ad3de0` widens only the exact RootLinks source passage: current/historical epochs, all three operations, and both `/usr` outcomes now follow `UsrRestored` -> `CandidatePreserveIntent` -> `CandidatePreserved`, with one reverse exchange and all five root-link identities unchanged. Route mutation coverage is 360 cases across two seams; admission rejects another 360 races spanning all five links. Common binding totals become 64 pre-effect, 64 post-effect, and 24 restart cases; NewState and ActivateArchived writer totals are 24/120/96/48 for success/storage fault/binding substitution/restart, and ActiveReblit totals are 32/160/128/64. Accepted commit `e35a2183` then admits only exact RootLinks NewState generation-15 `CandidatePreserved` and carries its non-Clone record-inode binding through a bound advance to generation-16 `FreshDbInvalidationIntent`, same-store successor validation, old-store destruction, and independent canonical reopen. Its proof covers 24 successes, 120 storage faults, 96 binding substitutions, 48 fresh-handle reopens, and 240 all-five-root-ABI races split across capture and final-revalidation seams. Accepted commit `a3fb25d3` independently admits exact RootLinks ActivateArchived generation-11 `CandidatePreserved` and carries its exact record-inode binding from capture through one bound advance to generation-12 `RollbackComplete`, same-store validation, and independent reopen. Its proof covers 24 successes, 120 storage faults, 96 binding substitutions, 48 fresh-handle reopens, and 360 all-five-root-ABI mutations across capture, fresh-namespace, and final-revalidation seams. Accepted commit `a05997d8`, with acceptance-gate follow-up `cfb5a70d`, independently admits exact RootLinks ActiveReblit generation-13 `CandidatePreserved`, carries its exact record-inode binding through capture, bound advance, same-store validation, and canonical reopen, and reaches generation-14 `RollbackComplete`. Its proof covers 24 successes, 120 storage faults, 96 binding substitutions, 48 fresh-handle reopens, and 240 all-five-root-ABI mutations split exactly 120 at `CaptureSandwich` and 120 at `FinalRevalidation`; the legacy fresh-namespace-capture race stays separate. The full endpoint performs exactly one reverse `/usr` exchange and one wrapper exchange, with no boot, database, non-journal namespace, finalization, or cleanup effect. `BootSyncStarted` remains disjoint and routes to `BootRepairRequired`. NewState stays byte-stable at generation 16, ActivateArchived at generation 12, and ActiveReblit at generation 14 because the RootLinks finalization gates remain closed. Fresh-handle reopen is not process-death evidence; RootLinks process death, terminal finalization, reboot, and power-loss durability remain unclaimed.
...
### Historical test-sealed NewState target selection, creation, normalization, and move reconciliation
  ...
  At that historical checkpoint, commit `0f041afe` kept the route behind a separate test-only seal. Accepted commit `e35a2183` now admits exact
  RootLinks-sourced NewState `CandidatePreserved` generation 15 and consumes its non-Clone record-inode binding through the bound advance to
  `FreshDbInvalidationIntent` generation 16. The writer validates the successor binding in the same store, destroys the old lock-bearing store, and requires
  an independent canonical reopen. Its 24 success, 120 storage-fault, 96 binding-substitution, and 48 fresh-handle-reopen cases cover both record
  epochs, three admitted candidate sources, both `/usr` outcomes, and both candidate outcomes. These handle reopens do not simulate process death.
  Another 240 cases mutate every canonical root link at capture or final revalidation. The end-to-end route keeps one reverse exchange; this entry invokes no
  database mutation or other non-journal effect, trigger, boot, completion, or
  finalization. A later entry leaves generation 16 byte-stable because every
  downstream NewState RootLinks gates stay closed. Commit `9adc2760` preserves the
  inventory-gate coverage while avoiding the host argument-size limit.
...
## Remaining recovery campaign
  ...
  The production ladder covers the authenticated `/usr` rollback prefix and exact RootLinks rollback for all three operations through candidate preservation and their next journal-only boundary. NewState advances generation 15 to generation-16 `FreshDbInvalidationIntent`; ActivateArchived advances generation 11 to generation-12 `RollbackComplete`; ActiveReblit advances generation 13 to generation-14 `RollbackComplete`. Each then stays byte-stable because its later RootLinks gate remains closed.
  Commit `e35a2183` proves the NewState journal-only edge with bound non-Clone record-inode identity, same-store and independent-reopen validation, 24/120/96/48 success/storage/binding/fresh-handle cases, and 240 all-link races at two seams. Commit `a3fb25d3` proves the independent ActivateArchived edge with the same 24/120/96/48 matrix shape and 360 all-five-root-ABI mutations across three seams. Accepted commit `a05997d8`, with acceptance gate `cfb5a70d`, proves the ActiveReblit bound generation-13 -> generation-14 edge with 24/120/96/48 cases and 240 all-five-link mutations split 120 `CaptureSandwich` + 120 `FinalRevalidation`; its legacy fresh-namespace-capture race remains separate. The complete ActiveReblit RootLinks route performs one reverse `/usr` exchange and one wrapper exchange. `BootSyncStarted` stays disjoint and routes to `BootRepairRequired`. These entries invoke no boot, database, non-journal namespace, finalization, or cleanup effect. Fresh-handle reopen is not process death; RootLinks terminal finalization, process death, reboot, and power-loss durability remain open.
