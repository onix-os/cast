...
## Remaining recovery campaign
  ...
  The exact receipt is strictly rederived immediately before one bound journal advance to `BootSyncStarted`; every injected fault seam classifies the exact durable predecessor or successor where possible and otherwise fails stop. Existing v1/v2 `BootSyncStarted` records retain their conservative journal-only startup route.
  Commit `b5928340` separately advances exact `BootRepairStarted` evidence to terminal `BootRepairUnverified` with zero boot calls; commit `406cabe5`'s Required -> Started, Started -> Complete/Unverified, and Complete -> `RollbackComplete` edges remain, but no startup entry performs repair or emits success.
  Accepted commits `4d1f8ceb` and `12f888e1` supply the aggregate attempt and disposable-UEFI-VM GPT alias/distinct remount evidence; `323f776e` supplies exact database promotion; `fffb79f7` consumes terminal aggregate authority into exact promoted-receipt classification. Their focused bridge/database/aggregate gates pass 20/20, 18/18, and 24/24.
  Accepted commit `58235570` consumes that exact non-`Clone` promoted authority through the typed `BootSyncComplete` successor and one inherited-deadline journal advance. It sandwiches the advance with exact promoted receipt/plan/terminal topology validation, then drops the old store, canonically reopens, proves the successor inode, captures a fresh binding, and validates again. All failures consume authority; fresh durable classification may expose only exact `BootSyncStarted` or `BootSyncComplete`, otherwise it fails stop. Its completion gate passes 9/9 tests covering 19 scenarios, with 20/20 terminal-promotion and 132/132 direct-journal dependencies.
  This proves component-level same-process persistence and fresh-handle reopen, not same-receipt startup recovery, live coordinator wiring, actual repair, boot replacement/deletion authority, selected-payload bootability or durability, device flush, interruption, reboot, or power-loss behavior. Caller-supplied keyed provenance claims remain inert rather than authenticated effect authority.
