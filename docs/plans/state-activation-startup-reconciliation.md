...
## Remaining recovery campaign
  ...
  Commit `92fa7aa0` retains the separate rollback route from exact ActiveReblit `CandidatePreserved` sourced from `BootSyncStarted` to `BootRepairRequired`, with zero boot calls.
  The versioned receipt, strict staging, aggregate publication, exact promotion, and same-process `BootSyncComplete` foundation remains as recorded in commits `5acba0ba`, `9f57157a`, `4d1f8ceb`, `12f888e1`, `323f776e`, `fffb79f7`, and `58235570`.
  Accepted commit `591e8fd7` recognizes only exact v3 promoted-receipt-backed ActiveReblit `BootSyncComplete` startup authority over the full state, selected active state, reservation, namespace, and bound source record.
  Accepted commit `facd9729` dispatches that authority before replacement cleanup, root-ABI normalization, and rollback admission.
  Ready authority derives the sole `CommitDecided` successor and performs exactly one bound advance.
  The successor is authenticated in the same store, after canonical reopen against the old binding, through a fresh binding, and by repeating the old-binding proof after fresh capture.
  Every handled entry returns `RecoveryPending` immediately, so neither cleanup nor redispatch occurs in that entry.
  Valid legacy v2 receipt-less, unpromoted, and stable-selection-mismatched `BootSyncComplete` evidence remains exactly pending and never falls through to rollback.
  The isolated promoted-receipt prerequisite passes 20/20; the leaf gate passes 6 authority, 4 dispatch, 5 evidence-race, 1 ten-case storage-fault matrix, and 2 namespace-classifier tests.
  `make check` and the source-LOC gate pass.
  The full focused aggregate dependency remains uncredited because unrelated existing two-second Gluon deadline tests flaked under cumulative load.
  Accepted commits `c0aae8bc`, `eed3d733`, and `1167383e` now admit exact promoted-receipt-backed `CommitDecided`, perform at most one cleanup exchange or zero-exchange Finish plus the shared durability suffix, and publish one bound `CommitCleanupComplete` successor before immediate `RecoveryPending`; the 6 authority/classifier + 3 effect + 4 persistence/dispatch tests, `make check`, and source-LOC pass. Accepted commit `84f16fd7` admits only exact v3 `CommitCleanupComplete`, retires a Promoted singleton receipt head once or accepts already-Retired evidence without another database mutation, retains and authenticates the current and optional predecessor immutable bodies, and advances once to the sole bound `Complete` successor before immediate `RecoveryPending`. Its 5 focused tests cover current/historical Apply and Finish with byte-stable `Complete` re-entry, after-commit retirement uncertainty, all five journal faults, six same-byte/new-inode binding windows, and focused database/namespace races; the focused Make gate, `make check`, and source-LOC pass. Accepted commit `68aebceb` then admits only exact forward ActiveReblit `Complete`, preserves exact Retired receipt state and the current and optional predecessor immutable bodies, consumes one record-bound terminal deletion, and retains the same locked store through final public absence and clean-startup admission. Its focused 5/5 finalization tests plus the prior 5/5 completion tests, `make check`, and source-LOC pass. Forward cleanup/finalization process death, reboot, and power-loss proof remain open, as do boot replacement/deletion and selected-payload bootability; long-term immutable-body GC stays in `FUTURE_PLAN.md`.
  ...
  At exact commit `07b917a73189563f02104455c937613ffe6b2e72`, UEFI guest `test` (machine ID `556a65c27e9b4150a9fb2b68f8693cdb`, boot ID `e875fab7-b970-4881-89d1-e87aa70acffb`) passed the separate guarded challenge/admission/campaign on `/dev/disk/by-path/virtio-pci-0000:07:00.0` -> `/dev/vdb`, diskseq `10`, `34359738368` bytes, after confirmed snapshot `os-tools-vdb-retry-20260721-07b917a7`: FAT32 `CASTTEST` retained `EFI/Linux` across sync/unmount/remount, ended unmounted with no runtime marker or lock, and passed read-only `fsck.fat -n` with 3 files and 3/2096126 clusters, while root `/dev/vda2` and live ESP `/dev/vda1` at `/boot/efi` remained untouched. The first `a6a834df` attempt failed closed after `mkfs.fat` auto-created fake-MBR child `/dev/vdb1`; its sentinel was preserved, exact state inspected, signatures explicitly recovered, and no mount/reboot occurred. Commit `07b917a7` adds `--mbr=n` and effective root-ownership validation. This is VFAT substrate evidence only, not payload publication, GPT ESP role, live-ESP mutation, reboot, or power-loss proof.
  ...
  Accepted commit `b0af65d6` applies that exact architecture to RootLinks NewState generation 18. Accepted commit `806003ac` applies it only to RootLinks ActiveReblit generation 14 with the deterministic coverage and delete-error policy recorded above. Commit `0a91c2ed` adds exact writer-reopen residue restoration; accepted commit `39456719` exercises it and all three RootLinks finalizers through genuine same-boot `SIGKILL`, without claiming reboot or power-loss behavior. Accepted commit `68aebceb` extends the separate forward ladder through exact `BootSyncComplete` -> `CommitDecided` -> `CommitCleanupComplete` -> `Complete` -> authenticated clean startup by consuming the final record binding in one terminal deletion while the same locked store preserves exact Retired receipt/body evidence and proves clean admission. Its focused finalization gate passes 5/5 alongside the prior 5/5 completion gate, `make check`, and source-LOC. Forward cleanup/finalization process death, reboot, and power-loss-equivalent proof remain open, as do
  boot replacement/deletion and selected-payload bootability; immutable-body GC stays in `FUTURE_PLAN.md`.
