...
## Remaining recovery campaign
  ...
  repair or emits success. Caller-supplied keyed provenance claims remain inert rather than authenticated effect authority. Accepted commits `4d1f8ceb` and `12f888e1` supply the aggregate attempt and disposable-UEFI-VM GPT alias/distinct remount evidence, while accepted commit `323f776e` supplies only exact database promotion. Terminal aggregate evidence is not yet allowed to invoke it; `BootSyncComplete`, same-receipt startup recovery, aggregate/live wiring, boot replacement/deletion authority, selected-payload bootability, reboot, and power-loss proof remain open.
