...
### Historical test-sealed NewState target selection, creation, normalization, and move reconciliation
  ...
  absence performs zero. Success/storage-fault/binding-substitution/fresh-handle matrices cover 48/240/192/96 executions; fresh handles are not process death.
  All-five-link races cover 240 capture, 240 pre-effect, 120 Applied post-attempt, 240 initial-persistence, and 240 final-revalidation cases. The endpoint
  performs exactly one reverse exchange and preserves every root-link target and identity. It invokes no trigger, boot, completion, or finalization effect.
  A later entry leaves generation 17 byte-stable because RootLinks completion and finalization remain closed. Same-boot `SIGKILL`, terminal finalization,
  reboot, and power-loss proof remain open; next is the separately admitted generation-17 `FreshDbInvalidated` -> generation-18 `RollbackComplete` edge.
...
## Remaining recovery campaign
  ...
  Accepted commit `7457b259` proves NewState's generation-16 -> generation-17 boundary with exact binding through capture, Apply-or-Finish effect, bound advance, same-store validation, and independent reopen. Present performs at most one exact removal and joint absence zero; success/storage/binding/fresh-handle totals are 48/240/192/96. All-five-link races total 240 capture + 240 pre-effect + 120 Applied post-attempt + 240 initial-persistence + 240 final-revalidation. Commit `a3fb25d3` proves ActivateArchived generation 11 -> 12; accepted `a05997d8` plus `cfb5a70d` proves ActiveReblit generation 13 -> 14. The endpoint performs one reverse `/usr` exchange, preserves all five link identities, and leaves Archived generation 12 and ActiveReblit generation 14 stable. Fresh handles are not process death; RootLinks same-boot `SIGKILL`, terminal finalization, reboot, and power-loss proof remain open. Next is NewState generation 17 -> generation-18 `RollbackComplete`, still separate from terminal deletion.
