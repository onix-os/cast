...
### Historical test-sealed NewState target selection, creation, normalization, and move reconciliation
  ...
  The existing 20-case `SIGKILL` proof remains exclusively generation 16 -> 17 and is not reboot or power-loss evidence. A recovery entry may naturally advance an already-canonical invalidation successor through generation 17 -> 18, but that adds no completion-route process-death claim. Next is an exact record-bound terminal-deletion primitive before operation finalization.
