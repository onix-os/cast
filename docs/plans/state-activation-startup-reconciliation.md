...
## Remaining recovery campaign
  ...
  repair or emits success. Caller-supplied keyed provenance claims remain inert rather than authenticated effect authority. Accepted commits `4d1f8ceb` and `12f888e1` supply the aggregate attempt and disposable-UEFI-VM GPT alias/distinct remount evidence; `323f776e` supplies exact database promotion; `fffb79f7` now consumes terminal aggregate evidence into that promotion while retaining exact promoted receipt classification and leaving the journal at `BootSyncStarted`. Its focused bridge/database/aggregate gates pass 20/20, 18/18, and 24/24. `BootSyncComplete`, same-receipt startup recovery, aggregate/live wiring, boot replacement/deletion authority, selected-payload bootability, reboot, and power-loss proof remain open.
