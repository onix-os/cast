...
## Admitted startup recovery ladder
  ...
  Exact RootLinks-sourced `UsrRestored` routes through candidate preservation. NewState advances generation 15 through 16 `FreshDbInvalidationIntent` and 17 `FreshDbInvalidated` to 18 `RollbackComplete`; ActiveReblit advances generation 13 to 14 `RollbackComplete`. Both then remain byte-stable because their RootLinks finalizers stay closed.
  ActivateArchived advances generation 11 to generation-12 `RollbackComplete`; exact `RootLinksComplete` generation 12 now finalizes through one record-bound deletion and the same locked-store clean handoff, while legacy `UsrExchangeIntent` and `UsrExchanged` admission remains intact.
  That RootLinks terminal lane proves fresh-handle source/absence recovery only, not the legacy Intent/Exchanged `SIGKILL` matrix, reboot, or power loss; private-detach residue still fails closed.
  ...
  Accepted commit `8f391985` closes that store blocker: its exact same-store non-`Clone` binding primitive detaches with `RENAME_NOREPLACE`, authenticates the retained inode/frame, unlinks once, syncs once, preserves foreign winners, and leaves private residue fail-closed. Accepted commit `a0966008` consumes it only for ActivateArchived `RootLinksComplete` generation-12 `RollbackComplete`, preserving legacy Intent/Exchanged admission. The exact binding is captured before database/namespace evidence, revalidated, and consumed once while the same locked store spans post-delete database, archived topology, all-five-link, repeated public-absence, and clean-handoff proof. NewState and ActiveReblit are not widened. RootLinks coverage is fresh-handle only: private-detach residue recovery, `SIGKILL`, reboot, and power-loss proof remain open.
...
### Historical test-sealed NewState target selection, creation, normalization, and move reconciliation
  ...
  Accepted commit `8f391985` supplies exact record-bound store deletion; `a0966008` consumes it only for exact RootLinks ActivateArchived generation 12, preserving Intent/Exchanged and retaining the same locked store through post-delete database/topology/all-five-link/public-absence proof and clean handoff. NewState and ActiveReblit remain unwidened. RootLinks has fresh-handle recovery only, not `SIGKILL`, reboot, or power-loss proof, until private-detach residue recovery exists.
...
## Remaining recovery campaign
  ...
  The production ladder covers the authenticated `/usr` rollback prefix and exact RootLinks rollback for all three operations through candidate preservation and their admitted later boundaries. NewState reaches generation-18 `RollbackComplete` and ActiveReblit generation-14 `RollbackComplete`, where both remain byte-stable. ActivateArchived reaches generation-12 `RollbackComplete` and now finalizes in a later bounded entry.
  ...
  Accepted commit `8f391985` supplies exact record-bound deletion. Accepted commit `a0966008` consumes it only for exact RootLinks ActivateArchived generation 12, preserving legacy Intent/Exchanged admission and the same locked store through post-delete database/topology/five-link/public-absence proof and clean handoff. NewState and ActiveReblit finalization remain closed. RootLinks ActivateArchived has fresh-handle source/absence recovery, but private-detach residue recovery and its own `SIGKILL`, reboot, and power-loss proof remain open. The earlier source set retains the exact NewState suffix through authenticated terminal absence and the ActiveReblit no-boot-repair suffix through shared clean admission.
  ...
  NewState and ActiveReblit are not widened. RootLinks has fresh-handle recovery only; private-detach residue recovery and its own `SIGKILL`, reboot, and power-
  loss proof remain open. The ladder still has no roll-forward executor, boot publisher, actual
...
- [ ] Add deterministic process-kill and fault-injection coverage at every
  ...
  ActiveReblit terminal deletion each add an exact 12-case matrix;
  ActivateArchived's matching matrix covers only legacy Intent/Exchanged, not
  its fresh-handle-only RootLinks finalizer. NewState and ActivateArchived candidate
