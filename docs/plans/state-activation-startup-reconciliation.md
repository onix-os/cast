...
## Admitted startup recovery ladder
  ...
  That RootLinks terminal lane remains fresh-handle evidence only, not the legacy Intent/Exchanged `SIGKILL` matrix, reboot, or power loss; exact private-detach residue can now be restored by the store's authenticated writer-reopen foundation.
...
## Remaining recovery campaign
  ...
  NewState and ActiveReblit are not widened. Commit `0a91c2ed` adds only exact writer-reopen residue restoration; it does not widen an operation finalizer or prove RootLinks `SIGKILL`, reboot, or power-
