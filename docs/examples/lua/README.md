...
# Lua package examples

> These are the canonical, Lua-authored examples — Cast's write path
> generates each one, and it re-decodes to the exact same value as its Gluon
> twin. Gluon is retained at full parity: the paired sources in
> [`../gluon/`](../gluon/) and the Gluon execution fixtures under
> `tests/fixtures/` stay, and per-domain parity tests pin the equivalence.

Machine-local boot inputs have three standalone examples:

| Example | What it demonstrates |
|---|---|
| [`boot-topology-aliases-esp.lua`](boot-topology-aliases-esp.lua) | One explicit ESP selector, including its canonical PARTUUID and authored mount point, used for both ESP and BOOT. |
| [`boot-topology-distinct-xbootldr.lua`](boot-topology-distinct-xbootldr.lua) | Explicit ESP and XBOOTLDR selectors with different canonical PARTUUIDs and authored mount points. |
| [`root-filesystem.lua`](root-filesystem.lua) | One explicit opaque root locator, from which Rust materializes exactly one `root=<value>` kernel token. |

The focused `make forge-active-reblit-boot-topology-intent-test` lane copies
both exact files to the fixed `etc/cast/boot-topology.lua` location beneath a
...
`/etc/cast/root-filesystem.lua` source. Its closed v1 value contains only
...
| Example | What it demonstrates |
|---|---|
| [`minimal`](packages/minimal/stone.lua) | A source-less package using only versioned defaults. |
| [`source-less-generated-config`](packages/source-less-generated-config/stone.lua) | A deterministic configuration artifact generated from authored data without an upstream source. |
| [`binary-release`](packages/binary-release/stone.lua) | An architecture-specific prebuilt archive with an explicit install contract. |
| [`platform-binary-factory`](packages/platform-binary-factory/stone.lua) | An explicit platform record selecting a prebuilt archive, target, and runtime loader without host discovery. |
| [`prebuilt-elf-runtime-contract`](packages/prebuilt-elf-runtime-contract/stone.lua) | A locked prebuilt ELF whose self-test, install, runtime ABI, stripping, and debug routing are explicit without compilation steps. |
| [`raw-script-package`](packages/raw-script-package/stone.lua) | A single locked, renamed, non-archive source installed with an explicit interpreter relation. |
| [`explicit-interpreter-suite`](packages/explicit-interpreter-suite/stone.lua) | Source-less Bash and Python commands whose inert shebang tokens are explicitly replaced, checked, and retained as exact runtime relations. |
| [`cmake`](packages/cmake/stone.lua) | CMake flags, checks, and typed build dependencies. |
| [`meson`](packages/meson/stone.lua) | Meson configuration and pkg-config dependencies. |
| [`cargo`](packages/cargo/stone.lua) | An offline Cargo build with features and explicit binaries. |
| [`go-module`](packages/go-module/stone.lua) | An offline vendored Go module with disabled network resolution, isolated caches, tests, and split documentation. |
| [`zig-project`](packages/zig-project/stone.lua) | A vendored Zig project with phase-local caches, tests, and runtime/development output relations. |
| [`python-module`](packages/python-module/stone.lua) | An offline Python wheel build with separate runtime executable, module, and test dependencies. |
| [`autotools`](packages/autotools/stone.lua) | Autotools flags, tests, and architecture selection. |
| [`desktop-application`](packages/desktop-application/stone.lua) | Desktop metadata, activation assets, runtime relations, and validation hooks. |
| [`font-family`](packages/font-family/stone.lua) | A data-only package with explicit font and documentation outputs. |
| [`header-only-library`](packages/header-only-library/stone.lua) | A compile-checked header-only library with development metadata and no runtime-library edge. |
| [`gettext-catalogs`](packages/gettext-catalogs/stone.lua) | Architecture-independent message catalogs compiled and checked by an explicit native tool closure. |
| [`firmware-bundle`](packages/firmware-bundle/stone.lua) | Architecture-independent firmware, device metadata, and license data installed without compilation. |
| [`conditionals`](packages/conditionals/stone.lua) | A pure package function driven by typed feature values. |
| [`optional-component-source-graph`](packages/optional-component-source-graph/stone.lua) | One typed feature adding a locked source, native tool, setup hook, and output as a coherent graph. |
| [`backend-choice-factory`](packages/backend-choice-factory/stone.lua) | A closed Lua variant selecting one mutually exclusive build and runtime backend. |
| [`release-source-factory`](packages/release-source-factory/stone.lua) | One explicit release record driving package metadata, source identity, and materialization names. |
| [`service-family-factory`](packages/service-family-factory/stone.lua) | One release and a closed member selector producing an exact daemon, client, or integration closure. |
| [`release-override`](packages/release-override/stone.lua) | An explicit attribute patch replacing package metadata and the complete source list together. |
| [`factory-override`](packages/factory-override/stone.lua) | Dependency-argument overrides followed by a typed attribute patch. |
| [`explicit-package-scope`](packages/explicit-package-scope/stone.lua) | Multiple explicit factories receiving one authored capability scope without reflection or recursive package-set magic. |
| [`explicit-package-set-extension`](packages/explicit-package-set-extension/stone.lua) | A non-recursive package-set extension passed explicitly into a source-less userspace bundle. |
| [`platform-factory`](packages/platform-factory/stone.lua) | A pure factory receiving explicit platform policy and dependency capabilities from local modules. |
| [`kernel-module-factory`](packages/kernel-module-factory/stone.lua) | A kernel-specialized package factory with an exact headers output, ABI release, module path, and target set. |
| [`layered-overrides`](packages/layered-overrides/stone.lua) | Ordered total package transformations with visible prepend, append, and scalar replacement semantics. |
| [`locked-template-substitution`](packages/locked-template-substitution/stone.lua) | A locked template specialized by one explicit Lua values record through typed `Run` steps and exact tool providers. |
| [`dependency-roles`](packages/dependency-roles/stone.lua) | Native, target, check, runtime, output, binary, library, and interpreter relations. |
| [`development-interface-closure`](packages/development-interface-closure/stone.lua) | An external named development output scoped only to `devel`, alongside one local planned library-output relation. |
| [`shared-capability-origins`](packages/shared-capability-origins/stone.lua) | One provider request retaining every distinct package, executable, and output origin in the frozen closure. |
| [`custom-steps`](packages/custom-steps/stone.lua) | Explicit `Run` and declared-program `Shell` steps. |
| [`manual-compiler-pipeline`](packages/manual-compiler-pipeline/stone.lua) | A fully explicit preprocess, compile, link, check, and install pipeline with split outputs. |
| [`hooks`](packages/hooks/stone.lua) | Structural pre/post hooks around a standard builder. |
| [`post-install-smoke-test`](packages/post-install-smoke-test/stone.lua) | An installCheck-style hook executing the artifact from the staged install tree. |
| [`multiple-sources`](packages/multiple-sources/stone.lua) | Archives, locked Git, renamed files, unpack policy, and destinations. |
| [`independent-vendor-source`](packages/independent-vendor-source/stone.lua) | Application and Cargo vendor closure as independently locked archives with strict offline source replacement and separately changeable identities. |
| [`explicit-git-subprojects`](packages/explicit-git-subprojects/stone.lua) | Three independently locked Git trees composed into one explicit subproject layout without recursive fetching. |
| [`native-codegen-target-library`](packages/native-codegen-target-library/stone.lua) | A build-platform generator and target-platform library kept in distinct typed dependency roles. |
| [`split-outputs`](packages/split-outputs/stone.lua) | Runtime, development, documentation, and root output rules. |
| [`typed-output-routing`](packages/typed-output-routing/stone.lua) | Ordered regular, executable, and symlink routing, with runtime FIFO creation declared through a packaged tmpfiles configuration. |
| [`userspace-role-factory`](packages/userspace-role-factory/stone.lua) | Closed workstation, server, and builder roles selected through an ordinary pure function rather than module merging. |
| [`variant-matrix-factory`](packages/variant-matrix-factory/stone.lua) | Two exhaustive typed axes forming one dependency and build-policy matrix without independent boolean drift. |
| [`conflicts`](packages/conflicts/stone.lua) | Typed conflicts, exclusions, provides, and path kinds. |
| [`options-tuning`](packages/options-tuning/stone.lua) | Toolchain choice, hardening, LTO, optimization, and package switches. |
| [`profiles-emul32`](packages/profiles-emul32/stone.lua) | Profile-specific builders and 32-bit dependency roles. |
| [`target-profile-specialization`](packages/target-profile-specialization/stone.lua) | Exact target-name selection replacing a package's builder, hooks, and dependency roles as one profile. |
| [`meta-package`](packages/meta-package/stone.lua) | A source-less package that declaratively composes a userspace. |
| [`output-policy-factory`](packages/output-policy-factory/stone.lua) | A higher-order package factory whose selected feature policy drives both the build and a typed multi-output graph. |
| [`output-tool-wrapper`](packages/output-tool-wrapper/stone.lua) | A wrapper generated by a program from a named dependency output. |
| [`runtime-environment-wrapper`](packages/runtime-environment-wrapper/stone.lua) | A source-less fixed wrapper binding an absolute program and exact plugin, data, trust, schema, and interpreter providers without ambient discovery. |
| [`generated-schema-library`](packages/generated-schema-library/stone.lua) | C sources generated and compiled by a tool bound to one named dependency output. |
| [`patch-series`](packages/patch-series/stone.lua) | An ordered typed patch series factored into a local Lua module. |
| [`external-patch-source`](packages/external-patch-source/stone.lua) | A separately locked raw patch applied to one structurally extracted primary source. |
| [`external-test-vectors`](packages/external-test-vectors/stone.lua) | A CMake check graph that explicitly admits a separately locked raw corpus through a declared copy capability. |
| [`pgo-workload`](packages/pgo-workload/stone.lua) | A structural offline training workload with multi-stage profile-guided optimization. |
| [`nodejs-vendored-application`](packages/nodejs-vendored-application/stone.lua) | A Node.js application built against a fully materialized local dependency tree without npm or registry access. |
| [`maven-application`](packages/maven-application/stone.lua) | A Java application built from an admitted Maven repository in strict offline mode. |
| [`realistic-daemon`](packages/realistic-daemon/stone.lua) | A larger daemon with hooks, services, multiple outputs, dependencies, and tuning. |
| [`system-integration-assets`](packages/system-integration-assets/stone.lua) | A service unit, sysusers, tmpfiles, udev, and polkit policy shipped as one declarative integration package. |
...
2. freezes every example with hermetic local source and repository fixtures,
   writes and reuses its exact `build.lock.lua`, and requires identical plan
...
These execution fixtures remain authored in Gluon — they are the retained
full-parity execution corpus under `tests/fixtures/gluon/`, unchanged by the
Lua-canonical examples above.
