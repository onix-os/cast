...
| [`development-interface-closure`](packages/development-interface-closure/stone.glu) | An external named development output scoped only to `devel`, alongside one local planned library-output relation. |
...
| [`independent-vendor-source`](packages/independent-vendor-source/stone.glu) | Application and Cargo vendor closure as independently locked archives with strict offline source replacement and separately changeable identities. |
...
| [`runtime-environment-wrapper`](packages/runtime-environment-wrapper/stone.glu) | A source-less fixed wrapper binding an absolute program and exact plugin, data, trust, schema, and interpreter providers without ambient discovery. |
