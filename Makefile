...
REQUIRE_EXECUTION ?= 0
FIXTURE ?= all
EXECUTION_FIXTURE_NAMES := autotools autotools-options cargo cargo-features cargo-vendored cmake custom daemon-generated factory-override generated-config generated-shell hooks-patch meson plugin-output post-install-smoke-test split userspace-profile
...
execution-fixtures: fixture-sources-check
	...
	@$(CARGO) test -p mason --lib \
		planner::hermetic_tests::bootstrap::pinned_bootstrap_manifest_is_bounded_and_index_authoritative -- \
		--exact --nocapture
	@echo "Resolving all seventeen execution fixtures against the pinned real Stone index..."
	@set -eu; \
	listed="$$( $(CARGO) test -p mason --lib \
