...
use super::super::{
    ...
    UsrRollbackCandidatePreserveSuccessorBindingError,
    arm_after_usr_rollback_candidate_preserve_successor_binding_check_before_reopen,
    arm_before_usr_rollback_candidate_preserve_successor_binding_revalidation,
...
#[test]
fn startup_usr_rollback_candidate_preserve_bound_advance_same_byte_replacements_never_succeed() {
                ...
                for origin in CandidateOrigin::ALL {
                    for usr_outcome in [RollbackActionOutcome::Applied, RollbackActionOutcome::AlreadySatisfied] {
                        ...
                        let hook = same_byte_different_inode_hook(
                            &fixture,
                            format!("bound-{boundary:?}-{historical}-{source:?}-{origin:?}-{usr_outcome:?}"),
                        );
        ...
        for source in Source::ALL {
            for origin in CandidateOrigin::ALL {
                for usr_outcome in [RollbackActionOutcome::Applied, RollbackActionOutcome::AlreadySatisfied] {
                    ...
                    let hook = same_byte_different_inode_hook(
                        &fixture,
                        format!("published-{historical}-{source:?}-{origin:?}-{usr_outcome:?}"),
                    );
                    arm_before_usr_rollback_candidate_preserve_successor_binding_revalidation(hook);
        ...
        for source in Source::ALL {
            for origin in CandidateOrigin::ALL {
                for usr_outcome in [RollbackActionOutcome::Applied, RollbackActionOutcome::AlreadySatisfied] {
                    ...
                    let hook = same_byte_different_inode_hook(
                        &fixture,
                        format!("reopened-{historical}-{source:?}-{origin:?}-{usr_outcome:?}"),
                    );
                    arm_after_usr_rollback_candidate_preserve_successor_binding_check_before_reopen(hook);
