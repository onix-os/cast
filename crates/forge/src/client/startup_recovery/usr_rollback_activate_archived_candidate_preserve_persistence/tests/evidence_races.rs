//! Final retained-evidence and journal-binding race contracts.
...
use super::super::{candidate_test_support::CandidateSource, test_fixture::OperationKind};
use super::support::{CandidateOrigin, Epoch, durable_authority, fixture_for_origin};
...
#[test]
fn startup_archived_candidate_preserve_persistence_rejects_reopened_and_cross_root_journals() {
    for epoch in Epoch::ALL {
        for origin in CandidateOrigin::ALL {
            ...
            let result = persist_usr_rollback_archived_candidate_preserve_and_reopen(second, authority);
            drop(reservation);
            ...
            let result = persist_usr_rollback_archived_candidate_preserve_and_reopen(foreign, authority);
            drop(reservation);
...
#[test]
fn startup_archived_candidate_preserve_persistence_final_evidence_races_fail_before_advance() {
    for epoch in Epoch::ALL {
        for origin in CandidateOrigin::ALL {
            for race in [
...
fn arm_final_race(fixture: &super::super::candidate_test_support::CandidatePreserveFixture, race: EvidenceRace) {
    let hook: Box<dyn FnOnce()> = match race {
        ...
        EvidenceRace::Installation => {
            ...
            let displaced = fixture
                ...
                .installation
                .root
                .join(".cast-archived-persistence-rebound");
            Box::new(move || {
        ...
        EvidenceRace::Namespace => {
            let mutation = fixture.namespace_change_hook("archived-persistence-final-namespace".to_owned());
            Box::new(move || {
                arm_before_archived_candidate_preserve_durable_post_revalidation_capture(mutation);
            })
        }
        EvidenceRace::Plan => {
            let changed = super::super::candidate_test_support::CandidatePreserveFixture::new(
