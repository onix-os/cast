    ...
    match advance {
        UsrRollbackDecisionAdvanceOutcome::Published(successor_binding) => match reopened {
            Ok((reopened, Some(actual))) if actual == decision => {
                let exact =
                    revalidate_reopened_decision_binding(&installation, &reopened, &successor_binding, &decision);
                drop(successor_binding);
                match exact {
        ...
        UsrRollbackDecisionAdvanceOutcome::SuccessorBindingFailed(binding) => match reopened {
            ...
            Err(reopen) => {
                Err(UsrRollbackDecisionPersistenceError::SuccessorRecordBindingAndReopen { binding, reopen })
            }
        },
    }
}
...
#[cfg(test)]
pub(crate) fn arm_after_usr_rollback_decision_successor_binding_check_before_reopen(hook: impl FnOnce() + 'static) {
