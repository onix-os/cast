...
use super::super::startup_reconciliation::{
    UsrRollbackArchivedCandidatePreserveDurableEffectAuthority, UsrRollbackCandidatePreserveAuthorityError,
};
...
pub(in crate::client) fn persist_usr_rollback_archived_candidate_preserve_and_reopen(
    ...
    let successor = match authority.candidate_preserved_successor() {
        ...
        Ok(successor) => {
            ...
            drop(journal);
            return Err(
                UsrRollbackArchivedCandidatePreservePersistenceError::UnexpectedSuccessor { phase: successor.phase },
            );
        }
        Err(source) => {
            drop(authority);
