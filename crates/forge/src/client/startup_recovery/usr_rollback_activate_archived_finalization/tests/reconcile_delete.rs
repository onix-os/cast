...
#[test]
fn activate_archived_finalization_preserves_exact_source_bound_delete_error() {
    ...
    assert!(matches!(
        error,
        UsrRollbackActivateArchivedFinalizationError::Delete(TransitionJournalRecordDeleteError::Storage {
...
#[test]
fn activate_archived_finalization_preserves_absent_bound_delete_error() {
    let fixture = FinalizationFixture::new();
    let journal = fixture.open_journal();
    let reservation = ActiveStateReservation::acquire().unwrap();
    let authority = fixture.capture_ready(&journal, &reservation);
...
#[test]
fn activate_archived_finalization_preserves_absent_error_when_post_delete_evidence_also_changes() {
    let fixture = FinalizationFixture::new();
    let journal = fixture.open_journal();
    let reservation = ActiveStateReservation::acquire().unwrap();
    let authority = fixture.capture_ready(&journal, &reservation);
