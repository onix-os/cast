//! RootLinksComplete root-ABI identity races at both authority revalidations.
...
use crate::{
    client::{
        ...
        startup_reconciliation::{
            arm_before_usr_rollback_complete_route_fresh_namespace_capture, fresh_db_invalidation_removal_call_count,
        },
        startup_recovery::{
...
fn root_abi_snapshot(root: &Path) -> [Option<RootAbiLinkIdentity>; 5] {
    ROOT_ABI.map(|(name, _)| match fs::symlink_metadata(root.join(name)) {
        ...
        Err(source) => panic!("inspect completion-route root ABI: {source}"),
    })
}
...
fn root_abi_mutation_hook(
    ...
    let root = fixture.fixture.fixture.fixture.installation.root.clone();
    let displaced_directory = tempfile::Builder::new()
        .prefix(".completion-route-root-abi-displaced-")
...
#[test]
fn startup_root_links_complete_route_initial_revalidation_rejects_all_root_abi_mutations() {
                    ...
                    for (name, target) in ROOT_ABI {
                        for mutation in RootAbiMutation::ALL {
                            ...
                            let fixture = if historical {
                                ...
                                RouteFixture::new(origin, Source::RootLinksComplete, usr_outcome, candidate_outcome)
                            };
...
#[test]
fn startup_root_links_complete_route_final_revalidation_rejects_all_root_abi_mutations() {
    let mut executions = 0;
    for historical in [false, true] {
        for origin in FreshDbOutcome::ALL {
            for usr_outcome in [RollbackActionOutcome::Applied, RollbackActionOutcome::AlreadySatisfied] {
                for candidate_outcome in CandidateResult::ALL {
                    for (name, target) in ROOT_ABI {
                        for mutation in RootAbiMutation::ALL {
                            ...
                            let hook = root_abi_mutation_hook(&fixture, name, target, mutation, case.clone());
                            arm_before_usr_rollback_complete_route_final_revalidation(move || {
                                arm_before_usr_rollback_complete_route_fresh_namespace_capture(hook);
                            });
