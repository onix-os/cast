...
use super::{
    ...
    support::{
        CandidateOutcome, Epoch, RouteFixture, candidate_move_count, capture_finalization_ready,
...
#[test]
fn startup_activate_archived_finalization_rejects_public_directory_and_lock_substitution() {
    for timing in [Timing::BeforeDelete, Timing::AfterDelete] {
        for directory in [true, false] {
            ...
            let displaced = root.join(if directory {
                ".cast/journal-displaced-by-archived-finalization-test"
            } else {
                ".cast/journal-lock-displaced-by-archived-finalization-test"
            });
...
#[test]
fn startup_activate_archived_finalization_rejects_hidden_entry_set_substitution() {
    for timing in [Timing::BeforeDelete, Timing::AfterDelete] {
        let fixture = exact_route();
        let terminal = persist_rollback_complete(&fixture);
        let journal = fixture.open_journal();
        ...
        let displaced = fixture
            ...
            .installation
            .root
            .join(".cast/journal/state-transition-displaced-by-archived-finalization-test");
...
fn startup_activate_archived_finalization_rejects_source_recreation_after_delete_and_absence_proof() {
    ...
    reset_candidate_observers();
    arm_after_usr_rollback_activate_archived_finalization_delete(move || write_new_private_file(&canonical, &bytes));

    let recreated = finalize_usr_rollback_activate_archived(journal, authority).unwrap_err();
...
fn arm_at(timing: Timing, hook: impl FnOnce() + 'static) {
    match timing {
        Timing::BeforeDelete => arm_before_usr_rollback_activate_archived_finalization_final_revalidation(hook),
        Timing::AfterDelete => arm_after_usr_rollback_activate_archived_finalization_delete(hook),
    }
}
...
fn assert_entry_set_error(timing: Timing, error: &UsrRollbackActivateArchivedFinalizationError) {
    let matched = matches!(
        (timing, error),
        (
            Timing::BeforeDelete,
