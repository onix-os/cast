//! Real-process restart proof for terminal ActiveReblit journal deletion.
...
use crate::{
    ...
    transition_journal::{
        ...
        RollbackAction, RollbackActionOutcome, StorageError, TransitionJournalStore, TransitionRecord,
...
use super::{
    ...
    support::{
        CandidateOrigin, Epoch, active_wrapper_path_at, assert_no_candidate_effects, build_active_at_wrapper_index,
        install_persistent_database, open_state_database, persist_rollback_complete, release_candidate_handles,
        reset_candidate_effect_observers,
    },
};

const TEST_NAME: &str = concat!(
    "client::startup_gate::usr_rollback_active_reblit::tests::finalization_process_kill::",
    "startup_active_reblit_finalization_process_kills_restart_cleanly",
);
const ROLE_ENV: &str = "CAST_FORGE_ACTIVE_REBLIT_FINALIZATION_KILL_ROLE";
const EPOCH_ENV: &str = "CAST_FORGE_ACTIVE_REBLIT_FINALIZATION_KILL_EPOCH";
const SOURCE_ENV: &str = "CAST_FORGE_ACTIVE_REBLIT_FINALIZATION_KILL_SOURCE";
const BOUNDARY_ENV: &str = "CAST_FORGE_ACTIVE_REBLIT_FINALIZATION_KILL_BOUNDARY";
const ROOT_ENV: &str = "CAST_FORGE_ACTIVE_REBLIT_FINALIZATION_KILL_ROOT";
const CONTROL_ENV: &str = "CAST_FORGE_ACTIVE_REBLIT_FINALIZATION_KILL_CONTROL";
...
impl ProcessRole {
    fn parse(value: &str) -> Self {
        match value {
            ...
            other => panic!("invalid ActiveReblit finalization process role {other:?}"),
        }
    }
...
impl ProcessEpoch {
    ...
    fn parse(value: &str) -> Self {
        match value {
            ...
            other => panic!("invalid ActiveReblit finalization epoch {other:?}"),
        }
    }
...
impl ProcessSource {
    ...
    fn parse(value: &str) -> Self {
        match value {
            ...
            other => panic!("invalid ActiveReblit finalization source {other:?}"),
        }
    }
...
impl FinalizationKillBoundary {
    ...
    fn parse(value: &str) -> Self {
        match value {
            ...
            other => panic!("invalid ActiveReblit finalization kill boundary {other:?}"),
        }
    }
    ...
    fn arm_kill(self) {
        match self {
            Self::FinalPreRevalidation => {
                arm_before_usr_rollback_active_reblit_finalization_final_revalidation(kill_self)
            }
            Self::CanonicalUnlinked => {
...
impl MatrixDimensions {
    fn for_case(epoch: ProcessEpoch, source: ProcessSource) -> Self {
        match (epoch, source) {
            (ProcessEpoch::Current, ProcessSource::Intent) => Self {
                ...
                candidate_origin: CandidateOrigin::Applied,
                wrapper_index: 0,
            },
            (ProcessEpoch::Current, ProcessSource::Exchanged) => Self {
                usr_outcome: RollbackActionOutcome::Applied,
                candidate_origin: CandidateOrigin::AlreadySatisfied,
                wrapper_index: 13,
            },
    ...
    fn validate(self, record: &TransitionRecord) {
        ...
        assert_eq!(
            ...
            recorded_action(self.candidate_origin.outcome())
        );
    }
}
...
impl ChildCase {
    ...
    fn terminal_record(&self) -> TransitionRecord {
        ...
        assert_eq!(
            ...
            expected_case,
            "external ActiveReblit process-kill control does not match the terminal case"
        );
        assert_wrapper(&self.root, &record, dimensions.wrapper_index);
        record
    }
}
...
#[test]
fn startup_active_reblit_finalization_process_kills_restart_cleanly() {
...
fn run_parent() {
    ...
    assert_eq!(
        cases, 12,
        "ActiveReblit finalization SIGKILL matrix must remain exactly 2 x 2 x 3"
    );
}
...
fn run_parent_case(epoch: Epoch, source: CandidateSource, boundary: FinalizationKillBoundary) {
    ...
    let database_before = ExistingCandidateDatabase::capture(&fixture.fixture.database, &terminal);
    let namespace_before = snapshot_startup_recovery_namespace(&root);
    let wrapper = active_wrapper_path_at(&fixture, dimensions.wrapper_index);
    assert_eq!(wrapper, wrapper_path(&root, &terminal, dimensions.wrapper_index));
    assert_wrapper(&root, &terminal, dimensions.wrapper_index);
    ...
    let crash_status = DeadlineChild::new(crash, "ActiveReblit finalization crash child").wait(CHILD_DEADLINE);
    assert_eq!(
        crash_status.signal(),
    ...
    let recovery_status = DeadlineChild::new(recovery, "ActiveReblit finalization recovery child").wait(CHILD_DEADLINE);
    assert!(
...
fn run_crash_child(case: &ChildCase, system: &MutableSystemCapabilities, terminal: &TransitionRecord) {
    ...
    arm_journal_update_durability_callback(JournalUpdateDurabilityBoundary::TemporaryFullySynced, || {
        panic!("ActiveReblit terminal crash path attempted a journal update")
    });
...
fn run_recovery_child(case: &ChildCase, system: &MutableSystemCapabilities, terminal: &TransitionRecord) {
    ...
    let database_before = ExistingCandidateDatabase::capture(database, terminal);
    let namespace_before = snapshot_startup_recovery_namespace(&case.root);
    reset_candidate_effect_observers();
    assert_no_candidate_effects();
    arm_journal_update_durability_callback(JournalUpdateDurabilityBoundary::TemporaryFullySynced, || {
        panic!("ActiveReblit terminal recovery attempted a journal update")
    });
    ...
    assert!(!canonical_path(&case.root).exists());
    assert_no_candidate_effects();
    assert_eq!(ExistingCandidateDatabase::capture(database, terminal), database_before);
    assert_eq!(snapshot_startup_recovery_namespace(&case.root), namespace_before);
    assert_wrapper(&case.root, terminal, case.dimensions().wrapper_index);
...
fn capture_database_at_root(root: &Path, terminal: &TransitionRecord) -> ExistingCandidateDatabase {
...
fn assert_wrapper(root: &Path, record: &TransitionRecord, wrapper_index: usize) {
    let wrapper = wrapper_path(root, record, wrapper_index);
    ...
    let usr_metadata = fs::symlink_metadata(&usr).unwrap();
    assert!(usr_metadata.is_dir(), "{} is not a directory", usr.display());
}
...
fn assert_journal_inventory(root: &Path, canonical_present: bool) {
    ...
    assert_eq!(names, expected, "unexpected ActiveReblit journal inventory");
}
...
fn required_unicode_env(name: &str) -> String {
    env::var_os(name)
        .unwrap_or_else(|| panic!("required ActiveReblit finalization environment {name} is missing"))
        .into_string()
        .unwrap_or_else(|_| panic!("ActiveReblit finalization environment {name} is not UTF-8"))
}
...
fn assert_separate_control_path(root: &Path, control: &Path) {
    ...
    assert!(
        ...
        "ActiveReblit process-kill control must remain outside the installation"
    );
}
