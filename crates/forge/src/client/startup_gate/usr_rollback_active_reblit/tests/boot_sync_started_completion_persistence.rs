...
use crate::{
    ...
    transition_journal::{
        TransitionRecord, arm_next_displaced_unlink_fault, arm_next_temporary_sync_fault,
...
use super::{
    boot_sync_complete_support::{
        BootSyncCompleteReadOnlySnapshot, boot_sync_started_fixture, capture_boot_sync_started_ready,
        open_boot_sync_complete_journal, same_byte_different_inode_hook,
    },
    support::{Epoch, assert_complete_route_journal_only, reset_complete_route_effect_observers},
};
...
impl ValidationHook {
    ...
    fn expected_stage(self) -> Option<ActiveReblitBootSyncStartedCompletionValidationStage> {
        match self {
            Self::FinalAuthority => None,
            Self::SameStore => Some(ActiveReblitBootSyncStartedCompletionValidationStage::SameStore),
            Self::BeforeReopen | Self::ReopenedOldBinding => {
                Some(ActiveReblitBootSyncStartedCompletionValidationStage::ReopenedOldBinding)
            }
            Self::OldBindingBeforeFreshCapture => {
                Some(ActiveReblitBootSyncStartedCompletionValidationStage::ReopenedOldBindingAfterFreshCapture)
            }
            Self::ReopenedFreshBinding => {
                Some(ActiveReblitBootSyncStartedCompletionValidationStage::ReopenedFreshBinding)
            }
        }
    }
}
...
fn promoted_boot_sync_started_persists_exact_completion_and_returns_reopened_store() {
    for epoch in Epoch::ALL {
        ...
        let (reopened, record) =
            persist_active_reblit_boot_sync_started_completion_and_reopen(journal, authority).unwrap();
...
fn promoted_boot_sync_started_all_five_update_faults_reconcile_source_or_successor() {
    ...
    for epoch in Epoch::ALL {
        for fault in JOURNAL_FAULTS {
            ...
            let error = match persist_active_reblit_boot_sync_started_completion_and_reopen(journal, authority) {
                Ok(_) => panic!("faulted BootSyncStarted completion returned a journal store"),
                Err(error) => error,
            };
...
fn promoted_boot_sync_started_all_six_binding_checks_reject_same_bytes_on_new_inode() {
    ...
    for epoch in Epoch::ALL {
        for hook in ValidationHook::ALL {
            ...
            reset_complete_route_effect_observers();
            let replacement =
                same_byte_different_inode_hook(&fixture, &format!("boot-sync-started-{epoch:?}-{hook:?}"));
            match hook {
                ValidationHook::FinalAuthority => {
                    arm_before_active_reblit_boot_sync_started_completion_final_revalidation(replacement)
                }
                ValidationHook::SameStore => {
                    arm_before_active_reblit_boot_sync_started_completion_same_store_validation(replacement)
                }
                ValidationHook::BeforeReopen => {
                    arm_after_active_reblit_boot_sync_started_completion_same_store_check_before_reopen(replacement)
                }
                ValidationHook::ReopenedOldBinding => {
                    arm_before_active_reblit_boot_sync_started_completion_reopened_validation(replacement)
                }
                ValidationHook::OldBindingBeforeFreshCapture => {
                    arm_after_active_reblit_boot_sync_started_completion_old_binding_validation(replacement)
                }
                ValidationHook::ReopenedFreshBinding => {
                    arm_before_active_reblit_boot_sync_started_completion_fresh_binding_validation(replacement)
                }
            }
