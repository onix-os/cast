...
impl<'reservation> UsrRollbackCompleteRouteAuthority<'reservation> {
    ...
    pub(in crate::client) fn advance_record_binding(
    ...
    ) -> Result<TransitionJournalRecordBinding, UsrRollbackCompleteRouteRecordAdvanceError> {
        ...
        journal
            ...
            .map_err(UsrRollbackCompleteRouteRecordAdvanceError::Storage)
    }
}
...
fn require_journal_record_binding(
    ...
    if !journal.has_record_store_binding(binding) {
        return Err(UsrRollbackCompleteRouteAuthorityErrorKind::JournalRecordBindingMismatch.into());
    }
    let cast = installation.retained_mutable_cast_directory()?;
    if journal.has_record_binding(cast, binding, record)? {
        Ok(())
    } else {
        Err(UsrRollbackCompleteRouteAuthorityErrorKind::JournalRecordBindingMismatch.into())
    }
}
...
impl From<StorageError> for UsrRollbackCompleteRouteAuthorityError {
    fn from(source: StorageError) -> Self {
        UsrRollbackCompleteRouteAuthorityErrorKind::Journal(source).into()
    }
}

#[derive(Debug, thiserror::Error)]
pub(in crate::client) enum UsrRollbackCompleteRouteRecordAdvanceError {
    #[error("revalidate exact NewState rollback-completion route authority before the bound journal advance")]
    Authority(#[from] UsrRollbackCompleteRouteAuthorityError),
    #[error("revalidate retained installation before the bound NewState rollback-completion route advance")]
    Installation(#[from] crate::installation::Error),
    #[error("advance the exact bound NewState rollback-completion route record")]
    Storage(#[source] StorageError),
}

#[derive(Debug, thiserror::Error)]
enum UsrRollbackCompleteRouteAuthorityErrorKind {
    #[error("NewState rollback-completion route authority lost its exact journal record binding")]
    JournalRecordBindingMismatch,
    #[error("capture or revalidate the exact NewState rollback-completion journal record")]
