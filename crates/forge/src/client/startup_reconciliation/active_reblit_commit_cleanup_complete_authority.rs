...
impl ActiveReblitCommitCleanupCompleteAuthority<'_> {
    pub(in crate::client) fn capture<'reservation>(
        ...
        record: &TransitionRecord,
    ) -> Result<ActiveReblitCommitCleanupCompleteAdmission<'reservation>, ActiveReblitCommitCleanupCompleteAuthorityError>
    {
        ...
        Ok(match captured {
            ...
            ActiveReblitCommitCleanupCompleteCapture::Deferred | ActiveReblitCommitCleanupCompleteCapture::Apply => {
    ...
    fn capture_with_record_binding<'reservation>(
        ...
        >,
    ) -> Result<ActiveReblitCommitCleanupCompleteCapture<'reservation>, ActiveReblitCommitCleanupCompleteAuthorityError>
    {
        ...
        if database_before != database_after || !record_plan_is_exact(record, &database_after.route) {
            return Err(ActiveReblitCommitCleanupCompleteAuthorityErrorKind::RouteEvidenceChanged.into());
        }
        require_exact_record_binding(installation, journal, &journal_record_binding, record)?;
        installation.revalidate_mutable_namespace()?;
...
impl<'reservation> ActiveReblitCommitCleanupCompleteAuthority<'reservation> {
    ...
    > {
        ...
        if !exact_complete_successor(&self.evidence.record, &successor, &self.evidence.database.route)? {
            return Err(ActiveReblitCommitCleanupCompleteRecordAdvanceError::UnexpectedSuccessor);
        }
...
impl ActiveReblitCommitCleanupCompletePostAdvanceAuthority<'_> {
    ...
    fn revalidate_successor(
        ...
        if !exact_complete_successor(&self.completed_record, successor, &self.database.route)
            .map_err(ActiveReblitCommitCleanupCompleteAuthorityErrorKind::Record)?
        {
            return Err(ActiveReblitCommitCleanupCompleteAuthorityErrorKind::UnexpectedSuccessor.into());
        }
        self.installation.revalidate_mutable_namespace()?;
        let before = require_exact_database(
            &self.database,
        ...
        self.namespace
            .revalidate_completed_namespace(&self.installation, &self.completed_record)?;
        let after = require_exact_database(
            &self.database,
            inspect_current_database(successor, &self.database.route, &self.state_db)?,
        )?;
        require_exact_active_state(successor, &self.installation, &self.active_state)?;
        if before != after {
            return Err(ActiveReblitCommitCleanupCompleteAuthorityErrorKind::DatabaseEvidenceChanged.into());
        }
        require_successor_binding(&self.installation, journal, binding, successor, mode)?;
        self.installation.revalidate_mutable_namespace()?;
        Ok(())
...
fn inspect_current_database(
    ...
    route: &ActiveReblitCommitCleanupCompleteRouteEvidence,
    state_db: &db::state::Database,
) -> Result<ActiveReblitCommitCleanupCompleteDatabaseInspection, ActiveReblitCommitCleanupCompleteAuthorityError> {
    match route {
        ActiveReblitCommitCleanupCompleteRouteEvidence::ReceiptBacked { pair, .. } => {
            inspect_receipt_backed_database(record, *pair, state_db)
        }
        ActiveReblitCommitCleanupCompleteRouteEvidence::NoBoot { .. } => inspect_no_boot_database(record, state_db),
    }
}
...
fn inspect_current_database_for_plan(
    ...
    match route {
        ...
        ActiveReblitCommitCleanupCompleteRoutePlan::NoBoot => inspect_no_boot_database(record, state_db),
    }
}
...
fn load_inert_no_boot_receipt_chain(
    record: &TransitionRecord,
    state_db: &db::state::Database,
) -> Result<db::state::CurrentExactPromotedBootPublicationReceiptChain, ActiveReblitCommitCleanupCompleteAuthorityError>
{
    let chain = state_db
    ...
    if matches!(
        ...
        return Err(ActiveReblitCommitCleanupCompleteAuthorityErrorKind::ReceiptChainMatchesNoBootTransition.into());
    }
    Ok(chain)
}
...
fn load_exact_promoted_receipt(
    ...
    record: &TransitionRecord,
    pair: crate::boot_publication::BootPublicationReceiptPair,
) -> Result<Option<db::state::BootPublicationReceiptState>, ActiveReblitCommitCleanupCompleteAuthorityError> {
    match state_db.load_exact_promoted_boot_publication_receipt_state(&record.transition_id, &pair) {
        ...
        ) => Err(ActiveReblitCommitCleanupCompleteAuthorityErrorKind::ReceiptCorrelation(source).into()),
        Err(
...
) -> Result<ActiveReblitCommitCleanupCompleteDatabaseEvidence, ActiveReblitCommitCleanupCompleteAuthorityError> {
    match actual {
        ActiveReblitCommitCleanupCompleteDatabaseInspection::Exact(actual) if actual == *expected => Ok(actual),
        _ => Err(ActiveReblitCommitCleanupCompleteAuthorityErrorKind::DatabaseEvidenceChanged.into()),
    }
}
...
fn require_exact_active_state(
    ...
    if snapshot.active() != Some(expected) {
        return Err(ActiveReblitCommitCleanupCompleteAuthorityErrorKind::ActiveSelectionChanged.into());
    }
    snapshot
        .revalidate(installation)
        .map_err(ActiveReblitCommitCleanupCompleteAuthorityErrorKind::ActiveState)?;
...
fn exact_route_plan(
    ...
    match (record.generation, record.options.run_boot_sync, receipt_correlation) {
        (_, true, Some(pair)) => Some(ActiveReblitCommitCleanupCompleteRoutePlan::ReceiptBacked(pair)),
        (12, false, None) if record.options.run_system_triggers && !record.options.archive_previous => {
...
fn record_plan_is_exact(record: &TransitionRecord, route: &ActiveReblitCommitCleanupCompleteRouteEvidence) -> bool {
...
fn exact_complete_successor(
    ...
    let successor_route_is_exact = match route {
        ActiveReblitCommitCleanupCompleteRouteEvidence::ReceiptBacked { pair, .. } => {
            successor.options.run_boot_sync && source_pair == Some(*pair) && successor_pair == Some(*pair)
        }
        ActiveReblitCommitCleanupCompleteRouteEvidence::NoBoot { .. } => {
...
fn require_exact_record_binding(
    ...
    if journal.has_record_store_binding(binding) && journal.has_record_binding(cast, binding, record)? {
        ...
        Err(ActiveReblitCommitCleanupCompleteAuthorityErrorKind::JournalRecordBindingChanged.into())
    }
}
...
fn require_successor_binding(
    ...
    let exact = match mode {
        ...
        BindingMode::Reopened => journal.has_reopened_record_binding(cast, binding, successor)?,
    };
    if exact {
        Ok(())
    } else {
        Err(ActiveReblitCommitCleanupCompleteAuthorityErrorKind::SuccessorRecordBindingChanged.into())
    }
}
...
impl From<ActiveReblitCommitCleanupNamespaceError> for ActiveReblitCommitCleanupCompleteAuthorityError {
