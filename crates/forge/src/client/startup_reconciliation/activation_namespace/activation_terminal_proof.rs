//! Independent namespace proof for the activation terminal phases.
//!
...
//! is exactly the shape an activation transition — `NewState` or
//! `ActivateArchived` — leaves behind once its candidate is live and its
...
//! is what makes the ActiveReblit terminal authorities large, and which neither
//! activation operation performs (`plans/future_impl.md` §1.1c).
...
#[derive(Debug)]
pub(in crate::client::startup_reconciliation) struct ActivationTerminalNamespaceInspection {
...
#[derive(Debug)]
pub(in crate::client::startup_reconciliation) struct ActivationTerminalNamespaceProof {
    before: NamespaceSnapshot,
    after: NamespaceSnapshot,
    layout: LayoutAlternative,
}

impl ActivationTerminalNamespaceInspection {
    pub(in crate::client::startup_reconciliation) fn begin(
        installation: &Installation,
        journal: &TransitionJournalStore,
        journal_record_binding: &TransitionJournalRecordBinding,
        expected: &TransitionRecord,
    ) -> Result<Self, ActivationTerminalNamespaceError> {
    ...
    pub(in crate::client::startup_reconciliation) fn finish(
        ...
        journal_record_binding: &TransitionJournalRecordBinding,
        expected: &TransitionRecord,
    ) -> Result<ActivationTerminalNamespaceProof, ActivationTerminalNamespaceError> {
        ...
        if before_layout != layout {
            return Err(ActivationTerminalNamespaceError::LayoutChanged);
        }
        require_exact_journal(installation, journal, journal_record_binding, expected)?;
        installation.revalidate_mutable_namespace()?;
        Ok(ActivationTerminalNamespaceProof {
            before: self.before,
            after,
            layout,
        })
...
impl ActivationTerminalNamespaceProof {
    ...
    pub(in crate::client::startup_reconciliation) fn revalidate(
        ...
        installation: &Installation,
        expected: &TransitionRecord,
    ) -> Result<(), ActivationTerminalNamespaceError> {
        installation.revalidate_mutable_namespace()?;
        self.before.revalidate_retained()?;
...
fn require_matching_fingerprints(
    ...
    if before.fingerprint() == after.fingerprint() {
        ...
        Err(ActivationTerminalNamespaceError::NamespaceChanged)
    }
}
...
fn require_exact_journal(
    ...
    if !journal.has_record_store_binding(binding) {
        return Err(ActivationTerminalNamespaceError::JournalRecordBindingChanged);
    }
...
pub(in crate::client::startup_reconciliation) enum ActivationTerminalNamespaceError {
