...
fn new_state_boot_tail_record(phase: Phase, previous: Option<i32>) -> TransitionRecord {
    let mut record = new_state_commit_decided_boot_record(previous);
    record.phase = phase;
    record.generation = crate::transition_journal::expected_forward_generation(
        &record,
        phase.forward().expect("a forward boot-tail phase"),
    )
    .expect("the phase is on the NewState boot chain");
    record
}

// Both journal-only edges of the first-install boot tail, and the shapes each
// must refuse. A predecessor means the reclaiming route still owes work.
#[test]
fn new_state_boot_tail_edges_admit_only_first_install_records() {
    for (phase, successor) in [
        (Phase::CommitDecided, Phase::CommitCleanupComplete),
        (Phase::CommitCleanupComplete, Phase::Complete),
    ] {
        assert!(
            new_state_boot_tail_edge_is_admissible_for_test(&new_state_boot_tail_record(phase, None), successor),
            "{phase:?} -> {successor:?} must admit a first install"
        );
        assert!(
            !new_state_boot_tail_edge_is_admissible_for_test(&new_state_boot_tail_record(phase, Some(41)), successor),
            "{phase:?} -> {successor:?} must refuse a record with a predecessor"
        );
        assert!(
            !new_state_boot_tail_edge_is_admissible_for_test(&new_state_boot_tail_record(phase, None), phase),
            "{phase:?} must refuse an unrelated successor"
        );
    }

    // The terminal deletion at `Complete` follows the same rule: only a first
    // install may take the journal-only route.
    assert!(new_state_boot_tail_terminal_is_admissible_for_test(
        &new_state_boot_tail_record(Phase::Complete, None)
    ));
    assert!(!new_state_boot_tail_terminal_is_admissible_for_test(
        &new_state_boot_tail_record(Phase::Complete, Some(41))
    ));
    assert!(!new_state_boot_tail_terminal_is_admissible_for_test(
        &new_state_boot_tail_record(Phase::CommitDecided, None)
    ));
}

fn new_state_commit_decided_boot_record(previous: Option<i32>) -> TransitionRecord {
    let mut record = record_at(Phase::CommitDecided);
    record.previous.id = previous;
    record.options.archive_previous = previous.is_some();
    record.previous_archive_slot = None;
    record.options.run_boot_sync = true;
    record.options.run_system_triggers = true;
    record.boot_publication_receipts = Some(test_boot_publication_receipt_pair());
    record.generation = crate::transition_journal::expected_forward_generation(&record, ForwardPhase::CommitDecided)
        .expect("CommitDecided is on the NewState boot chain");
    record
}
