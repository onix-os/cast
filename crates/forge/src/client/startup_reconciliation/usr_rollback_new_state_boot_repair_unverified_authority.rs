...
use super::super::{
    active_state_snapshot::ActiveStateReservation, startup_gate::UsrRollbackNewStateBootRepairUnverifiedSeal,
};
use super::{
...
pub(in crate::client) enum UsrRollbackNewStateBootRepairUnverifiedAdmission<'reservation> {
    NotApplicable,
    Deferred,
    Ready(UsrRollbackNewStateBootRepairUnverifiedAuthority<'reservation>),
}

/// Retained evidence authorizing only the next journal route decision.
pub(in crate::client) struct UsrRollbackNewStateBootRepairUnverifiedAuthority<'reservation> {
    installation: Installation,
    state_db: db::state::Database,
    record: TransitionRecord,
    database: UsrRollbackNewStateBootRepairUnverifiedDatabaseEvidence,
...
struct UsrRollbackNewStateBootRepairUnverifiedDatabaseEvidence {
    context: DatabaseEvidence,
    absence: db::state::ExactFreshTransitionAbsence,
}

enum DatabaseInspection {
    Exact(UsrRollbackNewStateBootRepairUnverifiedDatabaseEvidence),
    Incompatible(DatabaseEvidence),
}

impl<'reservation> UsrRollbackNewStateBootRepairUnverifiedAuthority<'reservation> {
    ...
    pub(in crate::client) fn capture(
        _startup_gate_seal: &UsrRollbackNewStateBootRepairUnverifiedSeal,
        installation: &Installation,
        ...
        UsrRollbackNewStateBootRepairUnverifiedAdmission<'reservation>,
        UsrRollbackNewStateBootRepairUnverifiedAuthorityError,
    > {
        if record.phase != Phase::BootRepairStarted || record.operation != Operation::NewState {
            return Ok(UsrRollbackNewStateBootRepairUnverifiedAdmission::NotApplicable);
        }
        ...
        let database_before = match inspect_current_database(record, state_db)? {
            ...
            DatabaseInspection::Incompatible(_) => {
                return Ok(UsrRollbackNewStateBootRepairUnverifiedAdmission::Deferred);
            }
        };
        let namespace_inspection = match UsrRollbackNewStateBootRepairNamespaceInspection::begin(installation, journal, record) {
            Ok(inspection) => inspection,
            Err(_) => return Ok(UsrRollbackNewStateBootRepairUnverifiedAdmission::Deferred),
        };
        ...
        installation.revalidate_mutable_namespace()?;
        Ok(UsrRollbackNewStateBootRepairUnverifiedAdmission::Ready(Self {
    ...
    pub(in crate::client) fn revalidate(
        &self,
        journal: &TransitionJournalStore,
    ) -> Result<(), UsrRollbackNewStateBootRepairUnverifiedAuthorityError> {
        ...
        if database_before != database_after || !boot_repair_required_plan_is_exact(&self.record) {
            return Err(UsrRollbackNewStateBootRepairUnverifiedAuthorityErrorKind::RouteEvidenceMismatch.into());
        }
        require_journal_record_binding(&self.installation, journal, &self.journal_record_binding, &self.record)?;
        self.installation.revalidate_mutable_namespace()?;
        Ok(())
    ...
    pub(in crate::client) fn advance_record_binding(
    ...
    ) -> Result<TransitionJournalRecordBinding, UsrRollbackNewStateBootRepairUnverifiedRecordAdvanceError> {
        ...
        journal
            ...
            .map_err(UsrRollbackNewStateBootRepairUnverifiedRecordAdvanceError::Storage)
    }
}
...
fn require_journal_record_binding(
    ...
    if !journal.has_record_store_binding(binding) {
        return Err(UsrRollbackNewStateBootRepairUnverifiedAuthorityErrorKind::JournalRecordBindingMismatch.into());
    }
    let cast = installation.retained_mutable_cast_directory()?;
    if journal.has_record_binding(cast, binding, record)? {
        Ok(())
    } else {
        Err(UsrRollbackNewStateBootRepairUnverifiedAuthorityErrorKind::JournalRecordBindingMismatch.into())
    }
}

/// Exact narrow plan accepted by the boot-repair boundary.
#[cfg(test)]
pub(in crate::client) fn usr_rollback_new_state_boot_repair_unverified_plan_is_exact_for_test(
    record: &TransitionRecord,
) -> bool {
...
fn boot_repair_required_plan_is_exact(record: &TransitionRecord) -> bool {
    ...
    record.operation == Operation::NewState
        && record.phase == Phase::BootRepairStarted
...
fn inspect_current_database(
    record: &TransitionRecord,
    state_db: &db::state::Database,
) -> Result<DatabaseInspection, UsrRollbackNewStateBootRepairUnverifiedAuthorityError> {
    let candidate = record
        .candidate
        .id
        .map(crate::state::Id::from)
        .ok_or(UsrRollbackNewStateBootRepairUnverifiedAuthorityErrorKind::RouteEvidenceMismatch)?;
    ...
    if exact_before != exact_after {
        return Err(UsrRollbackNewStateBootRepairUnverifiedAuthorityErrorKind::DatabaseChanged.into());
    }
    ...
    let evidence = UsrRollbackNewStateBootRepairUnverifiedDatabaseEvidence { context, absence };
...
fn database_pair_is_exact(
    record: &TransitionRecord,
    evidence: &UsrRollbackNewStateBootRepairUnverifiedDatabaseEvidence,
) -> bool {
...
fn require_exact_database(
    expected: &UsrRollbackNewStateBootRepairUnverifiedDatabaseEvidence,
    actual: DatabaseInspection,
) -> Result<
    UsrRollbackNewStateBootRepairUnverifiedDatabaseEvidence,
    UsrRollbackNewStateBootRepairUnverifiedAuthorityError,
> {
    match actual {
        DatabaseInspection::Exact(actual) if actual == *expected => Ok(actual),
        DatabaseInspection::Exact(_) => {
            Err(UsrRollbackNewStateBootRepairUnverifiedAuthorityErrorKind::DatabaseChanged.into())
        }
        DatabaseInspection::Incompatible(evidence) => {
            Err(UsrRollbackNewStateBootRepairUnverifiedAuthorityErrorKind::DatabaseIncompatible {
...
#[error(transparent)]
pub(in crate::client) struct UsrRollbackNewStateBootRepairUnverifiedAuthorityError(
    #[from] UsrRollbackNewStateBootRepairUnverifiedAuthorityErrorKind,
);

impl From<InspectionError> for UsrRollbackNewStateBootRepairUnverifiedAuthorityError {
    fn from(source: InspectionError) -> Self {
        UsrRollbackNewStateBootRepairUnverifiedAuthorityErrorKind::Inspection(source).into()
    }
}

impl From<db::state::ExactFreshTransitionInspectionError> for UsrRollbackNewStateBootRepairUnverifiedAuthorityError {
    fn from(source: db::state::ExactFreshTransitionInspectionError) -> Self {
        UsrRollbackNewStateBootRepairUnverifiedAuthorityErrorKind::ExactInspection(source).into()
    }
}

impl From<UsrRollbackNewStateBootRepairNamespaceError> for UsrRollbackNewStateBootRepairUnverifiedAuthorityError {
    fn from(source: UsrRollbackNewStateBootRepairNamespaceError) -> Self {
        UsrRollbackNewStateBootRepairUnverifiedAuthorityErrorKind::Namespace(source).into()
    }
}

impl From<crate::installation::Error> for UsrRollbackNewStateBootRepairUnverifiedAuthorityError {
    fn from(source: crate::installation::Error) -> Self {
        UsrRollbackNewStateBootRepairUnverifiedAuthorityErrorKind::Installation(source).into()
    }
}

impl From<StorageError> for UsrRollbackNewStateBootRepairUnverifiedAuthorityError {
    fn from(source: StorageError) -> Self {
        UsrRollbackNewStateBootRepairUnverifiedAuthorityErrorKind::Journal(source).into()
    }
}

#[derive(Debug, thiserror::Error)]
pub(in crate::client) enum UsrRollbackNewStateBootRepairUnverifiedRecordAdvanceError {
...
#[derive(Debug, thiserror::Error)]
enum UsrRollbackNewStateBootRepairUnverifiedAuthorityErrorKind {
...
#[cfg(test)]
pub(in crate::client) fn arm_between_usr_rollback_new_state_boot_repair_unverified_database_captures(
