...
//! superblock `rw`. Each pass also authenticates Linux MSDOS-family filesystem
//! magic against the exact retained destination descriptor identity. It
//! exposes only scalar facts after three complete later observation passes.
//! Every operation shares one caller-owned absolute deadline; no nested stage
//! replaces it with a fresh timeout.
//!
//! This is consistency evidence, not permission to mutate either destination.
//! It proves no GPT role, physical disk, persistence, durability, or
//! publication authority and never opens a raw block device. Requiring
//! `nosymfollow` gives future boot publication an effective Linux 5.10-or-newer
//! admission boundary without changing the generic `linux_fs` Linux 5.6
