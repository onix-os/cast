...
use crate::linux_fs::descriptor_boot_namespace::{
    ...
    RetainedBootNamespaceAssessmentLimits, RetainedBootNamespaceExpectedSource, assess_retained_boot_namespace_until,
};
use crate::linux_fs::mount_namespace::{
    PreparedMountNamespaceAnchor, RetainedBootFileMutationFingerprint, RetainedBootFilePublicationLimits,
    RetainedBootFilePublicationRequest, RetainedBootFileReplacementRequest, RetainedBootLeafAssessmentLimits,
    RetainedBootLeafAssessmentRequest, RetainedBootLeafAssessmentState, RetainedBootPublicationParent,
    RevalidatedTaskRootedAttachment, ValidatedRetainedBootFileReplacement, ValidatedRetainedBootLeafAssessment,
};

use super::{ActiveReblitBootOwnedLeafReplacementError, RevalidatedActiveReblitBootPublicationTarget};
...
impl FixtureOwnedReplacementAssessment {
    ...
    pub(super) fn replace(
        ...
        let selector = self.root.to_str().expect("fixture owned-replacement root is UTF-8");
        ...
        require_installed_assessment(&target, parent_components, installed, &assessment)?;
        let parent = target
            .retain_existing_boot_publication_parent_until(parent_components, deadline)
        ...
        let evidence = parent
            .replace_exact_boot_file_until(
                RetainedBootFileReplacementRequest::new(installed, replacement, owner),
                expected_source,
...
pub(in crate::client) fn fixture_owned_replacement_assessments_remaining() -> usize {
    ROOTS.with(|roots| roots.borrow().len())
}

pub(in crate::client) fn fixture_owned_replacement_validations_remaining() -> usize {
...
pub(super) fn take(
    ...
    let (root_path, validation_count) = ROOTS.with(|roots| roots.borrow_mut().pop_front())?;
    ...
    let metadata = root.metadata().expect("inspect fixture owned-replacement root");
...
fn validate_applied_at_root(
    ...
    let destination = target.destination();
    if metadata.dev() != destination.raw_device() || metadata.ino() != destination.inode() {
    ...
    let anchor =
        PreparedMountNamespaceAnchor::prepare().expect("prepare current-task namespace anchor for fixture validation");
    let attachment = anchor
        .revalidate()
    ...
    parent
        ...
        .map_err(ActiveReblitBootOwnedLeafReplacementError::LeafReplacementValidation)
}
...
fn require_validation_parent_identity(
    ...
    {
        Err(ActiveReblitBootOwnedLeafReplacementError::PublicationParentRootIdentity)
    } else {
        Ok(())
    }
}
...
fn require_installed_assessment(
    ...
    if assessment.canonical_leaf() != installed.canonical_leaf()
        || !assessment.parent_components().eq(parent_components.iter().copied())
    {
