...
use crate::{
    ...
    linux_fs::mount_namespace::{RetainedBootPublicationParent, ValidatedRetainedBootFileReplacement},
};

use super::{
    ActiveReblitBootOwnedLeafReplacementError, BoundReplacementPath, RevalidatedActiveReblitBootPublicationTarget,
    split_bound_replacement_path,
};
...
impl RevalidatedActiveReblitBootPublicationTarget<'_> {
    ...
    pub(in crate::client) fn validate_applied_owned_leaf_replacement(
        ...
        #[cfg(test)]
        if let Some(result) = super::fixture::validate_applied(self, path.parents(), evidence, self.deadline) {
            return result;
        }
...
fn require_evidence_matches_plan(
    ...
    if evidence.replacement_length() != output.expected_length()
        ...
        || evidence.replacement_sha256() != *output.expected_content_identity().as_bytes()
    {
        return Err(ActiveReblitBootOwnedLeafReplacementError::ReplacementPlanIdentity { plan_index });
    }
