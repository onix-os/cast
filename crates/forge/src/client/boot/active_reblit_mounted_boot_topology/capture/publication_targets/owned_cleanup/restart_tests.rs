...
use crate::{
    boot_publication::{
        ...
        BootPublicationReceiptBody, BootPublicationSha256, BootPublicationXxh3, CanonicalBootPublicationReceipt,
        prepare_boot_publication_receipt,
    },
    client::{
        active_reblit_mounted_boot_topology::{
            AliasFixture, arm_fixture_owned_cleanup_targets, fixture_owned_cleanup_targets_remaining,
        },
    ...
    linux_fs::{
        ...
        mount_namespace::{
            PreparedMountNamespaceAnchor, PreparedTaskRootedAttachment, RetainedBootFileStaleCleanupRequest,
...
fn output_with_claim(path: &str, bytes: &[u8], claim: BootPublicationOutputProvenanceClaim) -> BootPublicationOutput {
...
fn promote_receipt(database: &Database, receipt: &CanonicalBootPublicationReceipt) {
    assert_eq!(
        database.stage_boot_publication_receipt(receipt).unwrap(),
        BootPublicationReceiptStageOutcome::Staged,
    );
    assert_eq!(
        database.promote_boot_publication_receipt(receipt, deadline()).unwrap(),
...
fn receipt_replacement_reconstructs_fresh_authority_then_is_already_clean() {
    ...
    let applied = parent
        .replace_exact_boot_file_until(
            replacement_request("restart-replacement.efi", &predecessor, &installed, owner),
            &RetainedBootNamespaceExpectedSource::generated(INSTALLED_BYTES),
            RetainedBootFilePublicationLimits::default(),
            deadline(),
        )
        .unwrap();
    let sidecar = fixture.root.join("EFI/Linux").join(applied.sidecar_leaf());
    assert_eq!(fs::read(&sidecar).unwrap(), PREDECESSOR_BYTES);
    drop(applied);
    drop(parent);

    let path = split_cleanup_path(REPLACEMENT_PATH, "restart replacement", 0).unwrap();
    assert_eq!(
...
fn receipt_stale_cleanup_reconciles_canonical_detached_and_already_clean() {
    ...
    drop(parent);
    assert_eq!(
        reconcile_restart_stale_with_attachment(&view, identity, deadline(), 0, &path, &stale, owner,).unwrap(),
    ...
    parent
        ...
        .unwrap();
    let request = RetainedBootFileStaleCleanupRequest::new(output_request("restart-stale.efi", &stale), owner);
    let authority = parent
        ...
        .unwrap();
    let detached = fixture.root.join("EFI/Linux").join(authority.private_leaf());
    arm_stale_boot_file_stop_after_detach();
    assert!(
        parent
...
fn exact_receipt_plan_live_alias_targets_and_startup_seal_drive_restart_cleanup() {
    ...
    let replacement_index = plan
        .entries()
        .iter()
        .position(|entry| entry.predecessor_output().relative_path() == REPLACEMENT_PATH)
        .unwrap();
    let stale_index = plan
        .entries()
        .iter()
        .position(|entry| entry.predecessor_output().relative_path() == STALE_PATH)
        .unwrap();
    assert!(matches!(
    ...
    fs::set_permissions(fixture.publication_root(), fs::Permissions::from_mode(0o755)).unwrap();
    ...
    let real_attachment = real_anchor
        .revalidate()
        .unwrap()
        .prepare_task_rooted_attachment(fixture.publication_root().to_str().unwrap())
        .unwrap();
    let real_view = real_attachment.revalidate_against(&real_anchor).unwrap();
    let parent = retain_parent(&real_view);
    parent
        .publish_immutable_boot_file_until(
            output_request("restart-replacement.efi", replacement_entry.predecessor_output()),
            &RetainedBootNamespaceExpectedSource::generated(PREDECESSOR_BYTES),
            RetainedBootFilePublicationLimits::default(),
            deadline(),
        )
    ...
    parent
        .publish_immutable_boot_file_until(
            output_request("restart-stale.efi", stale_entry.predecessor_output()),
            &RetainedBootNamespaceExpectedSource::generated(STALE_BYTES),
            RetainedBootFilePublicationLimits::default(),
            deadline(),
        )
    ...
    let _cleanup_targets = arm_fixture_owned_cleanup_targets(fixture.publication_root().to_path_buf(), 4);
    let seal = ActiveReblitBootSyncStartedCleanupSeal::new_for_test(plan.promoted_receipt());
    assert_eq!(
        validated
            .reconcile_and_cleanup_restart_receipt_entry(&plan, replacement_index, &seal,)
            .unwrap(),
        ActiveReblitBootOwnedCleanupOutcome::RemovedReplacementRollback,
    );
    assert_eq!(
        validated
            .reconcile_and_cleanup_restart_receipt_entry(&plan, stale_index, &seal,)
            .unwrap(),
        ActiveReblitBootOwnedCleanupOutcome::RemovedOwnedStale,
    );
    for entry_index in [replacement_index, stale_index] {
        assert_eq!(
            validated
                .reconcile_and_cleanup_restart_receipt_entry(&plan, entry_index, &seal,)
                .unwrap(),
            ActiveReblitBootOwnedCleanupOutcome::AlreadyClean,
        );
    }
...
fn restart_admission_refuses_receipt_drift_noop_and_unowned_preserve() {
    ...
    require_restart_receipt_join(plan_receipt, plan_receipt, plan_receipt).unwrap();
    assert!(matches!(
        require_restart_receipt_join(fingerprint(0x62), plan_receipt, plan_receipt,),
        Err(ActiveReblitBootOwnedCleanupError::RestartTargetReceiptMismatch)
    ));
    assert!(matches!(
        require_restart_receipt_join(plan_receipt, fingerprint(0x63), plan_receipt,),
    ...
    for disposition in [
        ...
        assert!(matches!(
            ...
            Err(ActiveReblitBootOwnedCleanupError::RestartDispositionRefused { entry_index: 7 })
        ));
    }
    require_mutating_disposition(&ActiveReblitPromotedBootCleanupDisposition::ReplaceOwned, 8).unwrap();
    require_mutating_disposition(&ActiveReblitPromotedBootCleanupDisposition::DeleteOwnedStale, 9).unwrap();
}
