...
use super::{
    ...
    support::{Fixture, ROOT_LOCATOR, TreeSnapshot, authored_root, set_access_acl, set_test_xattr},
};
...
fn missing_machine_local_intent_is_a_hard_error_without_fallback() {
    ...
    assert!(matches!(
        fixture.prepare(),
        Err(ActiveReblitRootFilesystemIntentError::Missing { path }) if path == fixture.source_path()
    ));
    ...
    assert!(matches!(
        ...
        Err(ActiveReblitRootFilesystemIntentError::Missing { path }) if path == missing_parent.source_path()
    ));
}
...
fn symlink_fifo_socket_and_hardlink_sources_are_rejected_without_blocking() {
    ...
    let target = symlink_fixture.root.join("root-source-target");
    fs::write(&target, authored_root(ROOT_LOCATOR)).unwrap();
    symlink(&target, symlink_fixture.source_path()).unwrap();
    assert!(matches!(
        symlink_fixture.prepare(),
        Err(ActiveReblitRootFilesystemIntentError::UnsafeInode { .. })
    ));
    ...
    match UnixListener::bind(socket_fixture.source_path()) {
        ...
        Err(source) => panic!("bind private root-filesystem test socket: {source}"),
    }

    let hardlink_fixture = Fixture::new();
    hardlink_fixture.write_root(ROOT_LOCATOR);
    fs::hard_link(
...
fn source_and_ancestor_acls_or_xattrs_are_rejected_when_supported() {
    for target in ["source", "directory"] {
        ...
        xattr_fixture.write_root(ROOT_LOCATOR);
        ...
        acl_fixture.write_root(ROOT_LOCATOR);
        let path = if target == "source" {
...
fn invalid_intent_and_structural_failures_have_zero_mutation() {
    ...
    unsafe_fixture.write_root(ROOT_LOCATOR);
