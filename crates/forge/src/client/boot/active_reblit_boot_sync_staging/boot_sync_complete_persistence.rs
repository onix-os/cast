...
use super::{ActiveReblitBootSyncPromotedValidationError, StagedActiveReblitBootSync};
...
pub(in crate::client) struct CompletedStagedActiveReblitBootSync<'plan, 'inventory, Plan> {
...
pub(in crate::client) struct FreshCompletedStagedActiveReblitBootSync<'completed, 'client, 'plan, 'inventory, Plan> {
    completed: &'completed CompletedStagedActiveReblitBootSync<'plan, 'inventory, Plan>,
    _client: &'client Client,
}

impl<Plan> std::fmt::Debug for CompletedStagedActiveReblitBootSync<'_, '_, Plan> {
...
impl<'plan, 'inventory, Plan> CompletedStagedActiveReblitBootSync<'plan, 'inventory, Plan> {
    ...
    pub(in crate::client) fn revalidate_against<'completed, 'client>(
        ...
        FreshCompletedStagedActiveReblitBootSync<'completed, 'client, 'plan, 'inventory, Plan>,
    ...
    > {
        ...
        {
            return Err(ActiveReblitBootSyncCompleteValidationError::ClientCapabilityMismatch);
        }
    ...
    pub(in crate::client) fn reconcile_after_completed_validation_failure(
        self,
    ) -> Result<DurableActiveReblitBootSyncCompletionRecord, ActiveReblitBootSyncCompletionReconciliationError> {
        let Self {
            predecessor,
            record,
            record_binding,
        ...
        drop(record_binding);
        drop(journal);
        reconcile_completion_journal(&installation, &database, &predecessor, &record, &receipt)
    }
}

impl<'plan, 'inventory, Plan> FreshCompletedStagedActiveReblitBootSync<'_, '_, 'plan, 'inventory, Plan> {
    pub(in crate::client) const fn record(&self) -> &TransitionRecord {
    ...
    'input: 'plan,
{
    ...
    > {
        ...
        let successor = exact_boot_sync_complete_successor(&self.record, &self.receipt, pair)?;

        let repeated_deadline = require_promoted_staging_admission(&self, client)?;
        if repeated_deadline != deadline {
            return Err(ActiveReblitBootSyncCompletePersistenceError::PlanDeadlineMismatch {
                expected: deadline,
        ...
        match journal.advance_record_binding_until(cast, predecessor_binding, &successor, deadline) {
            Ok(successor_binding) => match finish_successful_completion_advance(
                ...
                Ok((journal, record_binding)) => Ok(CompletedStagedActiveReblitBootSync {
                    predecessor,
                    record: successor,
                    record_binding,
                    receipt,
                ...
                Err(validation) => {
                    match reconcile_completion_journal(&installation, &database, &predecessor, &successor, &receipt) {
                        Ok(durable) => Err(ActiveReblitBootSyncCompletePersistenceError::PostAdvanceValidation {
                            durable,
                            validation,
                        }),
            ...
            Err(source) => {
                ...
                match reconcile_completion_journal(&installation, &database, &predecessor, &successor, &receipt) {
                    Ok(durable) => {
                        Err(ActiveReblitBootSyncCompletePersistenceError::JournalAdvance { durable, source })
                    }
                    Err(reconciliation) => Err(
...
fn finish_successful_completion_advance(
...
) -> Result<(TransitionJournalStore, TransitionJournalRecordBinding), ActiveReblitBootSyncCompleteValidationError> {
    ...
    {
        return Err(ActiveReblitBootSyncCompleteValidationError::ReopenedSuccessorBindingChanged);
    }
    require_exact_started_record(predecessor, receipt, pair)?;
    ...
    validate_completed_successor(installation, database, &reopened, successor, &recaptured, receipt, pair)?;
...
fn validate_completed_successor(
    ...
    if !journal.has_record_store_binding(successor_binding) {
        return Err(ActiveReblitBootSyncCompleteValidationError::JournalCapabilityMismatch);
    }
    installation
        .revalidate_mutable_namespace()
        .map_err(ActiveReblitBootSyncCompleteValidationError::Installation)?;
...
fn reconcile_completion_journal(
    ...
    let (durable, expected) = match actual {
        ...
        Some(ref actual) if actual == successor => {
            require_exact_completed_record(successor, receipt, pair)?;
            (DurableActiveReblitBootSyncCompletionRecord::BootSyncComplete, successor)
        }
        actual => {
            return Err(ActiveReblitBootSyncCompletionReconciliationError::UnexpectedRecord {
    ...
    if !reopened
        ...
        .map_err(ActiveReblitBootSyncCompletionReconciliationError::Bind)?
    {
        return Err(ActiveReblitBootSyncCompletionReconciliationError::BindingChanged);
    }
    database
        .require_promoted_boot_publication_receipt(receipt)
...
#[cfg(test)]
pub(in crate::client) fn arm_before_completion_journal_reopen(callback: impl FnOnce() + 'static) {
...
pub(in crate::client) use commit_decision_handoff::{
    CommitCleanupCompleteStagedActiveReblitBootSync, CommitCleanupCompleteStagedActiveReblitBootSyncValidationError,
    ...
    CompleteStagedActiveReblitFinalizationError, CompletedStagedActiveReblitCommitDecisionError,
...
fn require_exact_record(
    ...
    if !crate::client::active_reblit_boot_sync_staging::supports_boot_sync(record.operation)
        ...
        || actual != Some(pair)
    {
        return Err(ActiveReblitBootSyncCompleteValidationError::RecordReceiptMismatch);
    }
    Ok(())
}
