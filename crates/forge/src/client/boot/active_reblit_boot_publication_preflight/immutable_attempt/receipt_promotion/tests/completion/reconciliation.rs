...
pub(super) const SCENARIO_COUNT: usize =
    JOURNAL_FAULT_SCENARIO_COUNT + WRONG_GENERATION_SCENARIO_COUNT + REOPEN_CONTENTION_SCENARIO_COUNT;

#[test]
fn completion_journal_faults_reconcile_only_exact_started_or_complete_without_token() {
    let cases: [(FaultArm, FaultAssert, DurableActiveReblitBootSyncCompletionRecord); JOURNAL_FAULT_SCENARIO_COUNT] = [
        (
    ...
    for (arm, assert_consumed, expected_durable) in cases {
        with_staged_alias_attempt!(|fixture,
            ...
            let expected = match expected_durable {
                DurableActiveReblitBootSyncCompletionRecord::BootSyncStarted => expected_record,
