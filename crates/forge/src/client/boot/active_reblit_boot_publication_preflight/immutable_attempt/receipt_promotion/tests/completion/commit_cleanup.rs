...
fn replaced_binding_and_uncertain_advance_return_no_cleanup_handoff() {
    with_exact_commit_cleanup_handoff!(|fixture, topology_fixture, _plan, client, committed| {
        let source = committed.record().clone();
    ...
    with_exact_commit_cleanup_handoff!(|fixture, topology_fixture, _plan, client, committed| {
        ...
        assert!(
            persist_commit_cleanup_with_assessments!(committed, &client, topology_fixture.publication_root(), 1)
                .is_err()
        );
