...
impl Drift {
    const ALL: [Self; 4] = [Self::Namespace, Self::Database, Self::Journal, Self::Plan];
}
...
fn pre_advance_wrong_client_and_four_drift_axes_never_reach_boot_sync_complete() {
    ...
    for scenario in PreAdvanceScenario::ALL {
        with_staged_alias_attempt!(|fixture,
                                    ...
                                    expected_record,
                                    _fingerprint| {
            let promoted = promote_alias_for_completion!(staged, &client, &plan, topology_fixture.publication_root());
            ...
            let wrong_database_before = wrong_client
                .as_ref()
                .map(|client| client.state_db.boot_publication_receipt_state().unwrap());
            match scenario {
                PreAdvanceScenario::WrongClient => {}
                PreAdvanceScenario::Drift(Drift::Namespace) => {
                    let leaf = first_output_path(&plan, topology_fixture.publication_root());
                    arm_after_initial_completion_handoff(move || fs::remove_file(leaf).unwrap());
                }
                ...
                PreAdvanceScenario::Drift(Drift::Journal) => {
                    let displaced = canonical.parent().unwrap().join("completion-pre-drift-displaced");
                ...
                PreAdvanceScenario::Drift(Drift::Plan) => {
                    arm_after_initial_completion_handoff(arm_bound_plan_collision_drift);
                }
            }
            let expected_assessments = match scenario {
                PreAdvanceScenario::WrongClient => 0,
                PreAdvanceScenario::Drift(Drift::Namespace) => 2,
                PreAdvanceScenario::Drift(Drift::Database | Drift::Journal | Drift::Plan) => 1,
            };
            reset_and_assert_no_legacy_boot_effect();
            let assessments = (expected_assessments != 0)
                .then(|| arm_exact_alias_assessments(topology_fixture.publication_root(), expected_assessments));

            let completion_client = wrong_client.as_ref().unwrap_or(&client);
            let error = promoted.persist_boot_sync_complete(completion_client).unwrap_err();
            ...
            drop(assessments);
            match scenario {
                PreAdvanceScenario::WrongClient => assert!(
                    matches!(
                        &error,
                        ActiveReblitBootSyncCompletionError::InitialHandoff(
                            ActiveReblitBootPostPromotionValidationError::PromotedStagedEvidence {
                                ...
                                source: ActiveReblitBootSyncPromotedValidationError::ClientCapabilityMismatch,
                            },
                        ),
                    ),
                    "{error:?}"
                ),
                PreAdvanceScenario::Drift(Drift::Namespace) => assert!(
                    matches!(
                        &error,
                        ActiveReblitBootSyncCompletionError::ImmediateHandoff(
                            ActiveReblitBootPostPromotionValidationError::TerminalEvidence {
                                ...
                                source: ActiveReblitBootTerminalEvidenceValidationError::DestinationNotExact {
                                    checkpoint: "immediate pre-persistence",
                                    plan_index: 0,
                ...
                PreAdvanceScenario::Drift(Drift::Database) => assert!(
                    matches!(
                        &error,
                        ActiveReblitBootSyncCompletionError::ImmediateHandoff(
                            ActiveReblitBootPostPromotionValidationError::PromotedStagedEvidence {
                                checkpoint: "immediate pre-persistence",
                                source: ActiveReblitBootSyncPromotedValidationError::ReceiptState(
                ...
                PreAdvanceScenario::Drift(Drift::Journal) => assert!(
                    matches!(
                        &error,
                        ActiveReblitBootSyncCompletionError::ImmediateHandoff(
                            ActiveReblitBootPostPromotionValidationError::PromotedStagedEvidence {
                                checkpoint: "immediate pre-persistence",
                                source: ActiveReblitBootSyncPromotedValidationError::BootSyncStartedBindingChanged,
                            },
                        ),
                    ),
                    "{error:?}"
                ),
                PreAdvanceScenario::Drift(Drift::Plan) => assert!(
                    matches!(
                        &error,
                        ActiveReblitBootSyncCompletionError::ImmediateHandoff(
                            ActiveReblitBootPostPromotionValidationError::TerminalEvidence {
            ...
            if scenario == PreAdvanceScenario::WrongClient {
                assert_eq!(fs::metadata(&canonical).unwrap().ino(), journal_inode_before,);
...
fn post_advance_namespace_database_journal_and_plan_drift_returns_no_completion_token() {
    ...
    for drift in Drift::ALL {
        with_staged_alias_attempt!(|fixture,
            ...
            let successor = expected_record.boot_sync_complete_successor(pair).unwrap();
            let outputs_before = publication_snapshot!(&plan, topology_fixture.publication_root());
            let database_before = fixture.state_db.boot_publication_receipt_state().unwrap();
            let canonical = canonical_journal(&fixture.installation);
            match drift {
                Drift::Namespace => {
                    let leaf = first_output_path(&plan, topology_fixture.publication_root());
                    arm_after_boot_sync_complete_persistence(move || fs::remove_file(leaf).unwrap());
                }
                Drift::Database => {
                    let database = fixture.state_db.clone();
                ...
                Drift::Journal => {
                    let displaced = canonical.parent().unwrap().join("completion-post-drift-displaced");
                    arm_after_boot_sync_complete_persistence(move || replace_file_identity(&canonical, &displaced));
                }
                Drift::Plan => {
                    arm_after_boot_sync_complete_persistence(arm_bound_plan_collision_drift);
                }
            }
            ...
            reset_and_assert_no_legacy_boot_effect();
            let assessments = arm_exact_alias_assessments(topology_fixture.publication_root(), expected_assessments);
            ...
            drop(assessments);
            match drift {
                Drift::Namespace => assert!(
                    matches!(
                        &error,
                        ActiveReblitBootSyncCompletionError::PostCompletion {
                            durable: DurableActiveReblitBootSyncCompletionRecord::BootSyncComplete,
                ...
                Drift::Database => assert!(
                    matches!(
                        &error,
                        ActiveReblitBootSyncCompletionError::PostCompletionAndReconciliation {
                            validation: ActiveReblitBootPostCompletionValidationError::CompletedStagedEvidence {
                                checkpoint: "post-persistence",
                                source: ActiveReblitBootSyncCompleteValidationError::ReceiptState(
                                    BootPublicationReceiptPromotionError::MissingPending,
                                ),
                            },
                            reconciliation: ActiveReblitBootSyncCompletionReconciliationError::ReceiptState(
                                BootPublicationReceiptPromotionError::MissingPending,
                            ),
                        },
                    ),
                ...
                Drift::Journal => assert!(
                    matches!(
                        &error,
                        ActiveReblitBootSyncCompletionError::PostCompletion {
                            durable: DurableActiveReblitBootSyncCompletionRecord::BootSyncComplete,
                            source: ActiveReblitBootPostCompletionValidationError::CompletedStagedEvidence {
                                checkpoint: "post-persistence",
                                source: ActiveReblitBootSyncCompleteValidationError::BindingChanged,
                            },
                        },
                    ),
                    "{error:?}"
...
fn final_return_revalidation_catches_late_drift_after_durable_completion() {
    ...
    with_staged_alias_attempt!(|fixture,
        ...
        let database_before = fixture.state_db.boot_publication_receipt_state().unwrap();
        let leaf = first_output_path(&plan, topology_fixture.publication_root());
        arm_before_final_completion_validation(move || fs::remove_file(leaf).unwrap());
        reset_and_assert_no_legacy_boot_effect();
        let assessments = arm_exact_alias_assessments(topology_fixture.publication_root(), 4);
