    ...
    with_staged_alias_attempt!(
        ...
        topology_fixture | {
            for output in plan.outputs() {
                ...
                let destination = topology_fixture.publication_root().join(output.relative_path());
                fs::create_dir_all(destination.parent().unwrap()).unwrap();
                set_safe_publication_parents(topology_fixture.publication_root(), output.relative_path());
        ...
        |fixture, topology_fixture, plan, _inventory, client, staged, _expected_record, fingerprint| {
            ...
            assert_eq!(promoted.already_exact_count(), plan.publication_count(),);
            assert_promoted_state(&fixture.state_db.boot_publication_receipt_state().unwrap(), fingerprint);
        }
    );
}
...
fn chained_predecessor_terminal_receipt_promotes_and_retains_the_chain() {
    with_staged_alias_attempt!(
        ...
        |fixture, topology_fixture, plan, _inventory, client, staged, _expected_record, fingerprint| {
            ...
            assert!(state.committed().unwrap().body().committed_predecessor().is_some(),);
        }
    );
}
...
fn mixed_terminal_evidence_promotes_once_and_preserves_journal_and_counters() {
    with_staged_alias_attempt!(
        ...
        |fixture, topology_fixture, plan, _inventory, client, staged, expected_record, fingerprint| {
            ...
            let record_path = fixture.installation.root.join(".cast/journal/state-transition");
            ...
            let journal = TransitionJournalStore::open_in_retained_cast(cast, &fixture.installation.root).unwrap();
            assert_eq!(
