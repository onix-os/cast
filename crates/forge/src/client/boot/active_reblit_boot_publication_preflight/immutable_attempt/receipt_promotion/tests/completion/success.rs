...
fn cleaned_promoted_typestate_preserves_exact_authority() {
    with_staged_alias_attempt!(|fixture,
                                ...
                                _expected_record,
                                fingerprint| {
        let terminal = publish_terminal_alias!(staged, &client, &plan, topology_fixture.publication_root());
...
fn first_adoption_completion_persists_only_exact_boot_sync_complete() {
    with_staged_alias_attempt!(|fixture,
        ...
        assert_eq!(
            completed.record().boot_publication_receipt_correlation().unwrap(),
            Some(pair),
        );
...
fn chained_already_promoted_completion_preserves_pair_bodies_and_outputs() {
    with_staged_alias_attempt!(
        ...
        deadline | {
            ...
            prior_record.transition_id = TransitionId::parse("fedcba9876543210fedcba9876543210").unwrap();
            let prior_receipt = plan
                .prepare_complete_boot_publication_receipt(inventory, &prior_record, None, claims)
                .unwrap();
            client.state_db.stage_boot_publication_receipt(&prior_receipt).unwrap();
            assert_eq!(
                client
                    .state_db
        ...
        |fixture, topology_fixture, plan, _inventory, client, staged, expected_record, fingerprint| {
            ...
            assert_eq!(
                fixture
                    .state_db
                    .promote_boot_publication_receipt(terminal.staged.receipt(), plan.input_deadline(),)
                    .unwrap(),
                BootPublicationReceiptPromotionOutcome::Promoted,
            );
            let promotion_assessments = arm_exact_alias_assessments(topology_fixture.publication_root(), 4);
            let promoted = terminal.promote_terminal_receipt(&client).unwrap();
            assert_eq!(fixture_boot_namespace_assessments_remaining(), 0);
            ...
            let completion_assessments = arm_exact_alias_assessments(topology_fixture.publication_root(), 4);
            let completed = promoted.persist_boot_sync_complete(&client).unwrap();
            assert_eq!(fixture_boot_namespace_assessments_remaining(), 0);
            ...
            let database_after = fixture.state_db.boot_publication_receipt_state().unwrap();
            assert_eq!(database_after, database_before);
            fixture
                .state_db
                .require_promoted_boot_publication_receipt(database_before.committed().unwrap())
                .unwrap();
            assert_eq!(
                publication_snapshot!(&plan, topology_fixture.publication_root()),
                outputs_before,
