...
macro_rules! with_exact_cleanup_complete_handoff {
    (|$fixture:ident, $topology_fixture:ident, $plan:ident, $client:ident,
        ...
        with_exact_commit_cleanup_handoff!(|$fixture, $topology_fixture, $plan, $client, committed| {
            reset_active_reblit_commit_cleanup_exchange_attempt_count();
            let $cleaned =
                persist_commit_cleanup_with_assessments!(committed, &$client, $topology_fixture.publication_root(), 2)
                    .unwrap();
...
macro_rules! with_exact_complete_handoff {
    (|$fixture:ident, $topology_fixture:ident, $plan:ident, $client:ident,
        ...
        with_exact_cleanup_complete_handoff!(|$fixture, $topology_fixture, $plan, $client, cleaned| {
            let $completed =
                persist_complete_with_assessments!(cleaned, &$client, $topology_fixture.publication_root(), 2).unwrap();
            $body
        });
    }};
}
...
fn uncertain_complete_advance_returns_no_handoff() {
    with_exact_cleanup_complete_handoff!(|fixture, topology_fixture, plan, client, cleaned| {
        ...
        assert!(persist_complete_with_assessments!(cleaned, &client, topology_fixture.publication_root(), 1).is_err());
...
fn complete_reopen_never_waits_behind_writer_blocked_journal_contender() {
    ...
    with_exact_cleanup_complete_handoff!(|fixture, topology_fixture, _plan, client, cleaned| {
        let expected = cleaned.record().forward_successor(None).unwrap();
