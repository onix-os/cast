...
fn leaf_drift_between_terminal_checks_fails_before_database_promotion() {
    with_staged_alias_attempt!(|fixture,
        ...
        arm_before_immediate_pre_promotion_terminal_check(move || fs::remove_file(leaf).unwrap());
        let assessments = arm_exact_alias_assessments(topology_fixture.publication_root(), 2);
...
fn leaf_drift_after_database_success_returns_outcome_but_no_success_authority() {
    with_staged_alias_attempt!(|fixture,
        ...
        let assessments = arm_exact_alias_assessments(topology_fixture.publication_root(), 3);
        let error = terminal.promote_terminal_receipt(&client).unwrap_err();
        assert_eq!(fixture_boot_namespace_assessments_remaining(), 0);
        drop(assessments);
        assert_eq!(
...
fn post_promotion_same_bytes_different_journal_inode_fails_stop() {
    with_staged_alias_attempt!(|fixture,
        ...
        let displaced = canonical.parent().unwrap().join("post-promotion-displaced-transition");
        ...
        arm_after_database_promotion(move || {
            ...
            let mode = fs::metadata(&callback_canonical).unwrap().permissions().mode();
            fs::rename(&callback_canonical, &callback_displaced).unwrap();
            fs::write(&callback_canonical, bytes).unwrap();
            fs::set_permissions(&callback_canonical, fs::Permissions::from_mode(mode)).unwrap();
        });
        let assessments = arm_exact_alias_assessments(topology_fixture.publication_root(), 3);
        let error = terminal.promote_terminal_receipt(&client).unwrap_err();
        assert_eq!(fixture_boot_namespace_assessments_remaining(), 0);
        ...
        assert!(matches!(
            error,
            ActiveReblitBootReceiptPromotionError::PostPromotion {
                source: ActiveReblitBootPostPromotionValidationError::PromotedStagedEvidence {
                    checkpoint: "post-promotion",
                    ..
                },
                ..
