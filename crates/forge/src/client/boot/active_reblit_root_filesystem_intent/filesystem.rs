...
fn discover_source(
    ...
    discovered.ok_or_else(|| ActiveReblitRootFilesystemIntentError::Missing { path: path.to_owned() })
}
...
fn discovery_error(error: RootDeclarationDiscoveryError) -> ActiveReblitRootFilesystemIntentError {
    match error {
        RootDeclarationDiscoveryError::Inspect { path, source } if source.raw_os_error() == Some(nix::libc::ELOOP) => {
        ...
        RootDeclarationDiscoveryError::NotRegular { path } => ActiveReblitRootFilesystemIntentError::UnsafeInode {
            path,
            reason: "root-filesystem source is not a regular file",
        },
        RootDeclarationDiscoveryError::Collision { .. } => ActiveReblitRootFilesystemIntentError::EvaluationContract {
