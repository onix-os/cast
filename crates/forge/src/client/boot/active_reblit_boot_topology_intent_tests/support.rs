...
use super::super::{
    ActiveReblitBootTopologyIntentError, PreparedActiveReblitBootTopologyIntent, boot_topology_intent_path,
};
...
impl Fixture {
    ...
    pub(super) fn source_path(&self) -> PathBuf {
        boot_topology_intent_path(&self.installation)
    }
    ...
    pub(super) fn write_alias(&self) {
        self.write_source(authored_alias(ESP_PARTUUID));
    }

    pub(super) fn prepare(
        &self,
    ) -> Result<PreparedActiveReblitBootTopologyIntent, ActiveReblitBootTopologyIntentError> {
        PreparedActiveReblitBootTopologyIntent::prepare(&self.installation)
    }
}
...
pub(super) fn set_test_xattr(path: &Path) -> io::Result<bool> {
    ...
    let result = unsafe {
        nix::libc::setxattr(
            encoded.as_ptr(),
            c"user.cast-boot-topology-test".as_ptr(),
            value.as_ptr().cast(),
            value.len(),
            0,
        )
