...
use crate::boot_publication::{
    ...
    BootPublicationRoot, BootPublicationSha256, BootPublicationXxh3, prepare_boot_publication_receipt,
};
use crate::db::state::{
...
fn replace_pending_fingerprint(connection: &mut SqliteConnection, fingerprint: BootPublicationReceiptFingerprint) {
    let changed = diesel::update(boot_publication_receipt_head::table)
        .set(boot_publication_receipt_head::pending_receipt_sha256.eq(Some(fingerprint.as_bytes().as_slice())))
        .execute(connection)
        .unwrap();
    assert_eq!(changed, 1);
}

fn replace_committed_fingerprint(connection: &mut SqliteConnection, fingerprint: BootPublicationReceiptFingerprint) {
    let changed = diesel::update(boot_publication_receipt_head::table)
        .set(boot_publication_receipt_head::committed_receipt_sha256.eq(Some(fingerprint.as_bytes().as_slice())))
        .execute(connection)
        .unwrap();
    assert_eq!(changed, 1);
}

fn restore_pending_head(connection: &mut SqliteConnection, receipt: &CanonicalBootPublicationReceipt) {
    ...
    let changed = diesel::update(boot_publication_receipt_head::table)
        .set((
            ...
            boot_publication_receipt_head::pending_transition_id.eq(Some(receipt.body().transition_id().as_str())),
            boot_publication_receipt_head::pending_receipt_sha256.eq(Some(pending.as_bytes().as_slice())),
        ))
        .execute(connection)
        .unwrap();
    assert_eq!(changed, 1);
