...
fn journal_coordinator_metadata_proof_is_owned_for_every_operation_and_uses_exact_os_info() {
    for candidate_kind in [
        ...
        match (candidate_kind, prepared) {
            (CandidateKind::NewState, PreparedStatefulTransitionCoordinator::NewStateIsolation(_)) => {}
            (
                CandidateKind::ActiveReblit,
    ...
    let failure = coordinator
        .finish_candidate_prepare(|_| {
            crate::transition_identity::CandidateMetadataOutputs::from_policy(
                COORDINATOR_OS_RELEASE,
                crate::system_model::snapshot_authorities(),
                b"// @generated by cast. DO NOT EDIT.\nindependently expected but incorrect snapshot\n".as_slice(),
            )
        })
        .unwrap_err();
    assert!(matches!(
