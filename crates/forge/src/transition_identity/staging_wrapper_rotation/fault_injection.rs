...
use super::model::{RetainedStagingWrapperRotationFaultPoint, StagingWrapperRotationError};
...
std::thread_local! {
    ...
    static BEFORE_JOURNAL_VALIDATION: std::cell::RefCell<Option<Box<dyn FnOnce()>>> =
        const { std::cell::RefCell::new(None) };
    static BEFORE_FINAL_PREPARATION_REVALIDATION: std::cell::RefCell<Option<Box<dyn FnOnce()>>> =
        const { std::cell::RefCell::new(None) };
}
...
#[cfg(test)]
pub(crate) fn arm_before_staging_wrapper_journal_validation(hook: impl FnOnce() + 'static) {
    BEFORE_JOURNAL_VALIDATION.with(|armed| *armed.borrow_mut() = Some(Box::new(hook)));
}

#[cfg(test)]
pub(crate) fn arm_before_staging_wrapper_final_preparation_revalidation(hook: impl FnOnce() + 'static) {
    BEFORE_FINAL_PREPARATION_REVALIDATION.with(|armed| *armed.borrow_mut() = Some(Box::new(hook)));
}

pub(super) fn before_journal_validation() {
    #[cfg(test)]
    BEFORE_JOURNAL_VALIDATION.with(|armed| {
...
pub(super) fn before_final_preparation_revalidation() {
    #[cfg(test)]
    BEFORE_FINAL_PREPARATION_REVALIDATION.with(|armed| {
        if let Some(hook) = armed.borrow_mut().take() {
            hook();
        }
    });
