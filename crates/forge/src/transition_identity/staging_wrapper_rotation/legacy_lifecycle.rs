//! Legacy no-journal lifecycle for non-destructive staging-wrapper replacement.
//!
...
impl RetainedStagingWrapperRotation {
    ...
    /// the final observation before every no-replace directory creation.
    ///
    ...
    /// Read-only proof of the pre-trigger reservation layout: the original
    /// fixed wrapper remains staged and the exact retained replacement remains
    /// empty at its durable quarantine name.
    pub(super) fn require_reserved(
        &self,
        installation: &Installation,
    ) -> Result<(), RetainedStagingWrapperRotationFailure> {
        self.revalidate_base(installation)
