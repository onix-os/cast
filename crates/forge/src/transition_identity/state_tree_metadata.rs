...
impl RetainedTreeStateId {
    ...
    pub(super) fn retain(store: &TreeMarkerStore, state: state::Id) -> Result<Self, super::Error> {
        ...
        let file = open_state_id(store, &path)?;
    ...
    pub(super) fn revalidate(
        ...
        let named = open_state_id(named_store, &self.path)?;
...
fn open_state_id(store: &TreeMarkerStore, path: &Path) -> Result<std::fs::File, super::Error> {
    openat2_file(
        store.retained_directory().as_raw_fd(),
        STATE_ID_NAME,
