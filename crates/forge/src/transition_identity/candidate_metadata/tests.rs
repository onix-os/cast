...
const SYNTHETIC_ACTIVE_SNAPSHOT: &[u8] = b"# active system snapshot\n{ packages = [] }\n";
...
fn synthetic_system_authorities() -> RegisteredGeneratedDeclarationAuthorities {
    let active =
        GeneratedDeclarationAuthority::new(synthetic_language("active", "one"), SYNTHETIC_ACTIVE_MARKER).unwrap();
    let alternate = GeneratedDeclarationAuthority::new(
        synthetic_language("alternate", "two"),
        b"# alternate system snapshot\n".as_slice(),
    )
    .unwrap();
    RegisteredGeneratedDeclarationAuthorities::new([active.clone(), alternate], active).unwrap()
}
...
fn registered_alternate_collision_prevents_the_entire_pair_publication() {
    ...
    fs::set_permissions(&alternate_path, Permissions::from_mode(0o644)).unwrap();
    let before = fs::symlink_metadata(&alternate_path).unwrap();

    let candidate = File::open(&candidate_path).unwrap();
    let error = CandidateMetadataPublication::begin(&candidate, &candidate_path)
        .unwrap()
        .publish(
            CandidateMetadataOutputs::from_policy(RELEASE, synthetic_system_authorities(), SYNTHETIC_ACTIVE_SNAPSHOT)
                .unwrap(),
        )
        .unwrap_err();
...
fn retained_pair_proof_rejects_a_later_registered_alternate() {
    ...
    create_candidate_root(&candidate_path);
    let candidate = File::open(&candidate_path).unwrap();
    let proof = CandidateMetadataPublication::begin(&candidate, &candidate_path)
        .unwrap()
        .publish(
            CandidateMetadataOutputs::from_policy(RELEASE, synthetic_system_authorities(), SYNTHETIC_ACTIVE_SNAPSHOT)
                .unwrap(),
...
fn authority_bound_snapshot_refuses_authored_fixed_name_before_pair_publication() {
    ...
    let error = CandidateMetadataPublication::begin(&candidate, &candidate_path)
        .unwrap()
        .publish(
            CandidateMetadataOutputs::from_policy(RELEASE, crate::system_model::snapshot_authorities(), SNAPSHOT)
                .unwrap(),
        )
        .unwrap_err();
...
fn existing_metadata_verification_rejects_wrong_independent_bytes_without_mutation() {
    ...
    let error = verification
        .prove(
            CandidateMetadataOutputs::from_policy(wrong_release, crate::system_model::snapshot_authorities(), SNAPSHOT)
                .unwrap(),
        )
        .unwrap_err();
