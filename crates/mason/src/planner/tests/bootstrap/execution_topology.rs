include!("execution_selection.rs");
...
mod pgo_workload_topology {
    use super::*;

    include!("execution_topology/pgo_workload.rs");
}

mod relation_policy_topology {
    use super::*;

    include!("execution_topology/relation_policy.rs");
}
...
fn assert_execution_fixture_topology(name: &str, plan: &stone_recipe::derivation::DerivationPlan) {
    ...
    if name == "pgo-workload" {
        pgo_workload_topology::assert_contract(plan);
        return;
    }
...
"#
        ...
        "relation-policy" => relation_policy_topology::expected(),
        "hooks-patch" => vec![
    ...
    if name == "relation-policy" {
        relation_policy_topology::assert_contract(plan);
    }
