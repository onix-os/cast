//! Canonical generated resolution data for recipe sources.
//!
//! The semantic model is versioned and validated independently of its
//! declaration engine. Typed adapters own language-runtime values and
//! canonical serialization without rewriting authored recipe source.
        ...
        for (index, source) in self.sources.iter().enumerate() {
            ...
            match source {
                SourceResolution::Archive(archive) => {
                    if !is_canonical_sha256(&archive.sha256) {
                        return Err(ValidationError::InvalidArchiveSha256 {
                            field: format!("sources[{index}].sha256"),
                            value: archive.sha256.clone(),
                        });
                    }
                }
                SourceResolution::Git(git) => {
                    ...
                    if !is_canonical_sha256(&git.materialization_sha256) {
...
mod tests {
    ...
    const OLD_V1_LOCK: &str = r#"// @generated by cast. DO NOT EDIT.
...
"#;
    ...
    fn restricted_decoder_round_trips_canonical_output() {
        ...
        let decoded = evaluate_source_lock(SOURCE_LOCK_FILE_NAME, &encoded).unwrap();
    ...
    fn decoder_rejects_schema_orders_and_incomplete_commits() {
        let canonical = canonical_source_lock(&sample_lock());
        let cases = [
            (
            ...
            (
                canonical.replace(ARCHIVE_SHA256, &"a".repeat(63)),
                "archive digest must be exactly 64 lowercase hexadecimal",
            ),
            (
        ...
        for (source, expected) in cases {
            let error = evaluate_source_lock(SOURCE_LOCK_FILE_NAME, &source).unwrap_err();
    ...
    fn decoder_applies_the_shared_transport_policy_to_every_locked_source_kind() {
        ...
        for (valid, invalid, field) in cases {
            ...
            let error = evaluate_source_lock(SOURCE_LOCK_FILE_NAME, &encoded).unwrap_err();
            assert!(matches!(
                error,
                DeclarationEvaluationError::Conversion(ValidationError::InvalidUrl {
                    field: ref found,
    ...
    fn decoder_rejects_actual_v1_and_missing_v2_materialization_digest() {
        assert!(evaluate_source_lock(SOURCE_LOCK_FILE_NAME, OLD_V1_LOCK).is_err());

        let missing = canonical_source_lock(&sample_lock())
            .lines()
            .filter(|line| !line.contains("materialization_sha256 ="))
            .collect::<Vec<_>>()
            .join("\n");
        let error = evaluate_source_lock(SOURCE_LOCK_FILE_NAME, &missing).unwrap_err();
        assert!(matches!(error, DeclarationEvaluationError::Evaluation(_)), "{error}");
    }

    #[test]
    fn decoder_preserves_malformed_source_diagnostics() {
        let error = evaluate_source_lock(SOURCE_LOCK_FILE_NAME, "{").unwrap_err();
        let DeclarationEvaluationError::Evaluation(diagnostic) = error else {
    ...
    fn unchanged_lock_is_not_replaced() {
        ...
        assert_eq!(fs::read_to_string(path).unwrap(), canonical_source_lock(&lock));
    }
