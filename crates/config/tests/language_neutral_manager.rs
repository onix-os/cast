...
#[test]
fn generated_save_uses_active_descriptor_and_delete_never_sniffs() {
    let temporary = tempdir().unwrap();
    ...
    let path = manager
        .save_declaration("profile", &value, &first)
        .unwrap();

    assert_eq!(path, temporary.path().join("bundle.d/profile.one"));
    assert_eq!(fs::read_to_string(&path).unwrap(), "# generated by first\none:saved\n");
    manager.delete_declaration("profile", &second).unwrap();
    assert!(path.exists());
    manager.delete_declaration("profile", &first).unwrap();
    assert!(!path.exists());
}
...
fn collisions_and_save_failures_remain_structured() {
    ...
    let first = FixtureAdapter::first(log);
    let error = manager
        .save_declaration(
            "profile",
            &FixtureConfig {
    ...
    assert!(matches!(
        manager.delete_declaration("profile", &first),
