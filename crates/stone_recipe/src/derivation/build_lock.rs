...
pub const BUILD_LOCK_SCHEMA_VERSION: u32 = 5;
...
mod tests {
    ...
    fn decoder_rejects_unsupported_schema() {
        let encoded = encode_build_lock(&sample_lock()).replacen("schema_version = 5", "schema_version = 6", 1);
        let error = decode_build_lock(BUILD_LOCK_FILE_NAME, encoded.as_bytes()).unwrap_err();

        assert!(matches!(
            error,
            BuildLockDecodeError::Validation(BuildLockValidationError::UnsupportedSchema { found: 6, .. })
        ));
    }

    #[test]
    fn decoder_rejects_pre_cast_schema_four() {
        let encoded = encode_build_lock(&sample_lock()).replacen("schema_version = 5", "schema_version = 4", 1);
        let error = decode_build_lock(BUILD_LOCK_FILE_NAME, encoded.as_bytes()).unwrap_err();

        assert!(matches!(
            error,
            BuildLockDecodeError::Validation(BuildLockValidationError::UnsupportedSchema { found: 4, .. })
        ));
    }
}
