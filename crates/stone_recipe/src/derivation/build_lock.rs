...
pub const BUILD_LOCK_SCHEMA_VERSION: u32 = 4;

const BUILD_LOCK_HASH_DOMAIN: &[u8] = b"os-tools-build-lock\0";
const REQUESTED_INPUTS_HASH_DOMAIN: &[u8] = b"os-tools-requested-inputs-v1\0";
...
/// One canonical provider request before Moss chooses an exact package/output.
///
...
mod tests {
    ...
    fn decoder_rejects_unsupported_schema() {
        let encoded = encode_build_lock(&sample_lock()).replacen("schema_version = 4", "schema_version = 5", 1);
    ...
    #[test]
    fn decoder_rejects_pre_provenance_schema_three() {
        let encoded = encode_build_lock(&sample_lock()).replacen("schema_version = 4", "schema_version = 3", 1);
        let error = decode_build_lock(BUILD_LOCK_FILE_NAME, encoded.as_bytes()).unwrap_err();
