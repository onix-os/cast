...
fn accepts_n_bytes_and_rejects_n_plus_one() {
    ...
    let source_path = directory.path().join("bounded.decl");
    fs::write(&source_path, b"1234").unwrap();
    let root = SourceRoot::new(directory.path()).unwrap();

    let accepted = root.load("bounded.decl", 4).unwrap();
    assert_eq!(accepted.logical_name(), "bounded.decl");
    assert_eq!(accepted.text(), "1234");

    fs::write(source_path, b"12345").unwrap();
    let rejected = root.load("bounded.decl", 4).unwrap_err();
    assert_eq!(rejected.category, DiagnosticCategory::Limit);
    assert_eq!(rejected.limit, Some(LimitKind::SourceSize));
    assert_eq!(rejected.source_name.as_deref(), Some("bounded.decl"));
}

#[test]
fn rejects_a_symlink_file() {
    let directory = tempfile::tempdir().unwrap();
    fs::write(directory.path().join("real.decl"), "42").unwrap();
    symlink("real.decl", directory.path().join("linked.decl")).unwrap();
    let root = SourceRoot::new(directory.path()).unwrap();

    let error = root.load("linked.decl", 16).unwrap_err();

    assert_eq!(error.category, DiagnosticCategory::Io);
    assert_eq!(error.source_name.as_deref(), Some("linked.decl"));
...
fn rejects_a_symlink_directory_component() {
    ...
    fs::write(real.join("value.decl"), "42").unwrap();
    symlink("real", directory.path().join("linked")).unwrap();
    let root = SourceRoot::new(directory.path()).unwrap();

    let error = root.load("linked/value.decl", 16).unwrap_err();

    assert_eq!(error.category, DiagnosticCategory::Io);
    assert_eq!(error.source_name.as_deref(), Some("linked/value.decl"));
    assert!(error.message.contains("cannot contain symbolic links"));
}
...
fn rejects_a_fifo_without_waiting_for_a_writer() {
    let directory = tempfile::tempdir().unwrap();
    let fifo = directory.path().join("blocking.decl");
    ...
    let worker = thread::spawn(move || {
        let _ = sender.send(worker_root.load("blocking.decl", 16));
    });
    ...
    assert_eq!(error.category, DiagnosticCategory::Io);
    assert_eq!(error.source_name.as_deref(), Some("blocking.decl"));
...
fn a_replaced_root_path_cannot_redirect_existing_loads() {
    ...
    fs::write(configured_path.join("value.decl"), "old-root").unwrap();
    ...
    fs::create_dir(&configured_path).unwrap();
    fs::write(configured_path.join("value.decl"), "replacement-root").unwrap();
    ...
    let source = root.load("value.decl", 32).unwrap();
    assert_eq!(source.logical_name(), "value.decl");
